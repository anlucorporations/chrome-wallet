...
> **Fase:** 3 — Desarrollo · **Versión:** 1.5 · **Estado:** 🚧 **EN CURSO — H1 ✅ COMPLETADO (2026-09-11) · H2 ✅ COMPLETADO (2026-09-11) · H3 ✅ COMPLETADO (2026-09-11) · H4 ✅ COMPLETADO (2026-09-12)**; hito en curso: **H5 — Redes y observabilidad**
> **Cambios de la v1.5 (resumen).** Se registra el **cierre de H4** (§3.4.9: fecha y **batería real** — `npx tsc -b` exit 0, `npm run build` exit 0 (6 entradas + `dist/manifest.json`), `npm run lint:prohibited` exit 0, **Vitest 587 passed / 41 ficheros**, **Playwright 44 passed / 0 failed**, `npm run forge:test` **10 passed / 0 failed** — y §6 marca **H4 ✅ COMPLETADO**) y **§3.4.10** cierra los **bloqueos** de la v1.4: **D-H4-E9** (`vite.config.ts` con `host: '127.0.0.1'`, corrección del `webServer` de la dApp), **D-H4-E10** (el relay ya no responde `-32603` al cerrarse el canal, la correlación viaja como `requestId` persistido y la resolución empujada vuelve con el `id` de la página) y **dos defectos medidos nuevos** en el re-render de la ventana única —**D-H4-E11** (el empuje del cuerpo iba por `chrome.tabs.sendMessage`, que **no** alcanza a una página `chrome-extension://`) y **D-H4-E12** (carrera entre la respuesta de `SIGN_RESPONSE` y el empuje de la siguiente solicitud, que dejaba el botón «Aprobar» deshabilitado)—. Quedan **CUMPLIDOS** los 13 criterios de H4 (`CA-RF-35`, `CA-RF-41` y RNF-08 pasan de **PARCIAL** a **CUMPLIDO**). El hito en curso pasa a **H5**.
> **Cambios de la v1.4 (resumen).** Se registra la **puesta en verde del cableado de H4** y su estado REAL de entonces: la ruta de los **6 métodos aprobables** queda operativa en producción (`rpc/catalog.ts` + `rpc/router.ts`), con **Vitest 587 passed / 41 ficheros**, **Playwright 42 passed / 2 failed**, `tsc -b`, `build`, `lint:prohibited` y `forge:test` (10/10) en verde, y una **prueba real de extremo a extremo** archivada (`evidencia/H4/extremo-a-extremo-2026-09-12.{json,png}`: 1 ETH, UNA ventana con preview decodificada y contador, hash `0x21f9…33a6`, recibo `status 0x1`, cola vacía y marca en vuelo liberada). Se abre la **§3.4.10** con los puntos medidos —cableado (**D-H4-E1**), regresión de H3 **verificada en verde** (**D-H4-E2**), observaciones ya corregidas (**D-H4-E5**, **D-H4-E6**), dos defectos de las ventanas de la UI (**D-H4-E7**, **D-H4-E8**), la **corrección del comando de Anvil** del arnés (**D-H4-E3**) y los **dos bloqueos** (**D-H4-E9**, **D-H4-E10**)—; **H4 no se marcó completado** en esa versión.
> **Cambios de la v1.3 (resumen).** Se registra el **cierre de H3** (§3.3.9: fecha y **batería real** — `npx tsc -b` exit 0, `npm run build` exit 0 (6 entradas + `dist/manifest.json`), `npm run lint:prohibited` exit 0, **Vitest 403 passed / 28 ficheros**, **Playwright 37 passed / 0 failed**, `rg` de `chrome.storage` y de `from 'ethers'` en `src/popup` y `src/connect` → **0**, y `cast chain-id` = **31337**, `cast block-number` **≥ 1** y `cast balance 0xf39F…2266` = **10 000 ETH** — y §6 marca **H3 ✅ COMPLETADO**), y se abre la nueva **§3.3.10 «Cierre de H3: desviaciones aceptadas»** con los **5 puntos** descubiertos al poner la batería en verde —`accountsChanged` ausente al cambiar de cuenta activa desde el popup (**D-H3-A**), semántica de la sesión por origen decidida como la **opción (a)** (**D-H3-B**), `INTERNAL_METHODS` capturado en la evaluación de módulo de `senderGuard` (**D-H3-C**), **defecto del arnés** por la falta de `webServer` en `playwright.config.ts` (**D-H3-D**) y la **nota operativa de Anvil** (`--silent` no arranca con `Start-Process` y redirección de salida)—, cada uno con su motivo y su impacto (decisiones **DEC-57..DEC-61** de `estado_proyecto.md`).
> **Cambios de la v1.2 (resumen).** Se registra el **cierre de H2** (§3.2.9: fecha y **batería real** — `npx tsc -b` exit 0, `npm run build` exit 0, `lint:prohibited` exit 0, **Vitest 290 passed / 18 ficheros**, **Playwright 23 passed / 0 failed**, cobertura de `crypto/` **95,13 %** y de `shared/validation/` **97,27 %**, `grep` de `chrome.storage` y de `ethers` en `src/popup` → **0**— y §6 marca **H2 ✅ COMPLETADO**), y se abre la nueva **§3.2.10 «Cierre de H2: desviaciones aceptadas»** con las **5 correcciones/deviaciones** descubiertas al poner los 23 E2E en verde —revelado autorizado por origen y no por `sender.tab` (**D-H2-G**), orden de guardas del reset y consulta previa del popup (**D-H2-H**), aviso de éxito del reset tras el desmontaje de la vista (**D-H2-I**), borrado del portapapeles aplazado por la falta de foco (**D-H2-J**) y las correcciones del arnés E2E (**D-H2-K**)—, cada una con su motivo y su impacto (decisiones **DEC-52..DEC-56** de `estado_proyecto.md`).
...
|---|---|---|---|
| **Anvil** (red primaria) | `127.0.0.1:8545`, chainId `31337` (`0x7a69`), `--host 127.0.0.1`, `--allow-origin "chrome-extension://<ID>,http://localhost:5174,http://127.0.0.1:5174"` (nunca `*`; lista separada por comas, **no** repetible) | `anvil --version` dentro de `>=1.0.0 <2.0.0` y `cast chain-id --rpc-url http://127.0.0.1:8545` → `31337` | H2, H3, H4, H5, H6 |
...
| **H2 ✅ COMPLETADO (2026-09-11)** | Cartera, cuentas HD, importación, recepción, persistencia, recuperación y reset | RF-01, RF-02, RF-03, RF-04, RF-05, RF-07, RF-09, RF-10, RF-11, RF-33, RF-50 | CU-01..CU-09, CU-30, CU-32, CU-33 | M8, M9, M10, M12, M13, M28, M29, M33, M34, M39, M40, M41, M46, M57, M58, M59, M60, M61, M62, M63 | `Vitest: mnemonic`, `derivation`, `importPrivateKey`, `accounts`, `state`, `integrity`, `secretsExport`, `revealHygiene`, `revealClipboard`, `validation`, `reset`, `qr` · `E2E: 01-onboarding`, `02-cuentas`, `03-recibir`, `05-persistencia`, `06-reset`, `16-validacion`, `25-recuperacion` · **Vitest 290 passed / 18 ficheros** · **E2E 23 passed / 0 failed** | **24 h / 6 sesiones** |
| **H3 ✅ COMPLETADO (2026-09-11)** | Provider inyectado con alias, conexión de dApps, lectura y eventos contra Anvil | RF-13, RF-14, RF-15, RF-16, RF-17, RF-18, RF-25, RF-26, RF-27, RF-36, RF-45 | CU-08, CU-10, CU-17..CU-22, CU-26, CU-31 | M2, M3, M4, M5, M6, M20, M21, M22, M23, M26, M27, M35, M36, M37, M38, M44, M47, M48, M49, M55, M56, `test.html` (4 flujos) | `Vitest: inject`, `naming`, `errors`, `eip1193`, `messaging`, `manifest`, `accounts`, `sessions`, `polling`, `rpcRetry`, `rateLimit`, `originFrame`, `setCurrentAccount`, `importOrder` · `E2E: 07-provider`, `08-eventos`, `09-conectar`, `13-revocar`, `14-polling`, `27-rpc-caido`, `28-iframe-hostil` · **Vitest 403 passed / 28 ficheros** · **E2E 37 passed / 0 failed** | **20 h / 5 sesiones** |
...
#### 3.2.9 Definición de terminado (H2) — ✅ **COMPLETADO el 2026-09-11**
...
**Cierre verificado (2026-09-11).** Hito **COMPLETADO**. Batería real ejecutada con **Anvil en `127.0.0.1:8545` (chainId 31337)** y `dist/` reconstruido por el `global-setup`; evidencia archivada en `RepoTecnico/evidencia/H2/` y acta `ACTA_H2.md`:

| Comando | Resultado real (2026-09-11) | Evidencia |
|---|---|---|
| `npx tsc -b` | **exit 0** — strict, 0 errores (incluye `src/`, `test/` y `e2e/`) | `typecheck-2026-09-11.log` |
| `npm run build` | **exit 0** — 6 entradas en `dist/` + `dist/manifest.json` | `build-2026-09-11.log` |
| `npm run lint:prohibited` | **exit 0** — 0 dependencias prohibidas, 0 `fetch(` propio, 0 `chrome.storage.sync`, 0 `codecrypto_`, 0 colores fuera de `tokens.css` y 0 `ethers` en el popup | `lint-prohibited-2026-09-11.log` |
| `npm run test` (Vitest) | **exit 0** — **290 passed / 18 ficheros**, 0 failed | `vitest-2026-09-11.log` |
| `npm run coverage` | **exit 0** — `src/background/crypto/` **95,13 %** de líneas (488/513) y `src/shared/validation/` **97,27 %** (214/220): umbral ≥ 80 % **cumplido** | `coverage-2026-09-11.json`, `vitest-coverage-2026-09-11.log` |
| `npm run test:e2e` (Playwright) | **exit 0** — **23 passed / 0 failed / 0 skipped / 0 flaky** (23 specs de H1+H2) | `playwright-2026-09-11.log`, `e2e-2026-09-11.json`, `e2e-global-setup-2026-09-11.{log,json}` |
| `rg -n "chrome\.storage" src/popup` | **0 coincidencias** (RNF-14: el popup no toca el almacén) | `_verificacion-resumen.log` |
| `rg -n "from 'ethers'" src/popup` | **0 coincidencias** (`CA-RT-02`) | `_verificacion-resumen.log` |

> **Nota de host.** En este equipo Windows no hay `grep` instalado; los dos últimos comandos se ejecutan con **ripgrep** (`rg`), el mismo motor de búsqueda, y `rg` devuelve **exit 1 = 0 coincidencias** en ambos casos.

**Evidencia por flujo (§7.4.1.f):** `01-onboarding-2026-09-11.{json,png}`, `25-recuperacion-2026-09-11.png` y `reset-2026-09-11.json`.

#### 3.2.10 Cierre de H2: desviaciones aceptadas

Cinco correcciones se descubrieron al poner en verde los **23 E2E** del hito. Las cinco se decidieron **con la especificación como árbitro** (CU-30, `CA-RF-50`, RNF-09/RNF-14/RNF-22/RNF-23 y `documento_tecnico.md` §3.8/§3.9/§5.1.1/§5.2) y **ninguna rebaja un criterio de aceptación**: el efecto observable exigido se mantiene. Quedan registradas como **DEC-52..DEC-56** en `estado_proyecto.md`.

| # | Desviación / corrección | Motivo real (medido en H2) | Impacto y corrección |
|---|---|---|---|
| **D-H2-G** | El revelado se autoriza por **origen + ruta del popup**, no por `sender.tab` | `canRevealInContext` exigía `tabId === null`, pero el popup del `action` y la página `index.html` abierta en una pestaña llegan **con `sender.tab`** (la misma observación que ya documentó **D-H2-B** para el router). Medido: `wallet_revealSecret` respondía **`4200 methodNotAllowedInContext`** SIEMPRE, de modo que `CA-RF-50` era inalcanzable por la UI | Se elimina la condición `tabId === null`; se conservan `isExtensionContext` **y** `REVEAL_ALLOWED_ROUTES = ['index.html']` (una página web sigue respondiendo `4200`). Con sesión de dApp vigente el SW responde ahora **`-32000 accountInUseByDapp`** (DEC-45/R-09a). `src/background/crypto/secrets.ts` (M12); alineadas 2 aserciones de `secretsExport.spec.ts` |
| **D-H2-H** | **Orden de guardas** del reset en el SW y **consulta previa** del popup | Dos defectos: (a) `handleResetWallet` comprobaba `confirm` **antes** que las guardas, invirtiendo el orden de **CU-30** (cola vacía → sin transacción en vuelo → confirmación) y dejando el bloqueo `-32000` inalcanzable con `confirm: false`; (b) el popup abría el diálogo destructivo **sin** consultar las guardas, cuando CU-30 paso 3 y §3.9 exigen abrirlo **solo** con ambas en verde | M33 aplica el orden y `handleResetWallet` traduce `blocked → -32000` y `cancelled → 4001` sin cortocircuitar (`src/background/rpc/catalog.ts`). El popup consulta con `wallet_resetWallet { confirm: false }` (**`probeResetGuards`**, `src/popup/walletState.ts`), pinta el `-32000` del SW con el número exacto de pendientes y **no** abre el diálogo. `src/popup/views/SecurityView.tsx` |
| **D-H2-I** | El aviso de éxito del reset sobrevive al **desmontaje** de la vista | El reset deja la cartera vacía: `App` vuelve al formulario inicial y `SecurityView` se desmonta, así que el aviso «Cartera reseteada» (estado local de la vista) desaparecía. CU-30 paso 6 exige volver al inicio **informando** (nunca en silencio) | El aviso se eleva a `App` (`flash`) y se pinta con `tone="success"` sobre el estado inicial: `src/popup/App.tsx` + `onResetDone` de `SecurityView` |
| **D-H2-J** | **Borrado del portapapeles aplazado** cuando el ocultado ocurre sin foco | Medido con el navegador real (página de la extensión con `clipboardRead`/`clipboardWrite` y el documento sin foco): `navigator.clipboard.writeText('')` → `NotAllowedError: Document is not focused` y `document.execCommand('copy')` devuelve `true` **sin** modificar el portapapeles. Es decir: con el disparador de **pérdida de foco** el borrado inmediato es **físicamente imposible** y la semilla quedaría en el portapapeles, incumpliendo `CA-RF-50` («al ocultarse … borra el portapapeles si aún contiene la semilla») | Al ocultar sin foco se guarda **solo la huella SHA-256** del valor (el mecanismo `clipboardHash` que ya prescribe `diccionario_datos.md` §3.10), se descarta el valor (§3.8 regla 4) y el borrado se ejecuta **en cuanto la ventana recupera el foco**, con comparación por huella: **nunca** se borra a ciegas lo que el usuario haya copiado entretanto (§3.8 regla 5 / P-20). Si el borrado diferido falla, se registra y se avisa igual que antes (`-32603 clipboardFailure`). `src/popup/views/SecurityView.tsx`. **Desviación declarada** de §3.8 regla 5 («no se reintenta si la API falla»): el reintento es **uno solo**, al recuperar el foco, y solo si el contenido sigue coincidiendo |
| **D-H2-K** | Correcciones del **arnés E2E** (`e2e/`), no del producto | (1) `seedWallet` sembraba `truekeate_current_account: 'idx:0'` con `accounts: []` y `truekeate_accounts: []`: M13 lo clasifica como **cartera dañada** (`current-account-missing`, RNF-22, ya probado en `integrity.spec.ts`) y el popup pintaba «Wallet dañada» en lugar del estado vacío; (2) `getByRole('button', { name: 'Importar frase' })` resolvía a **2 elementos** (el que abre el formulario y el `submit`); (3) «una segunda pestaña pasa a primer plano» **no produce pérdida de foco** en Chromium headless (Playwright mantiene la emulación de foco: `hasFocus` sigue siendo `true` y no hay `blur`); (4) el reset elimina `truekeate_settings` (§3.9), así que el aviso no descartable de RNF-23 **vuelve** a mostrarse | (1) `seedWallet` solo escribe las claves coherentes con lo sembrado (`e2e/fixtures/h2.ts`); (2) selector acotado al `form` (`01-onboarding.spec.ts`); (3) nuevo `loseFocusToOtherPage` (`e2e/fixtures/extension.ts`): desactiva la emulación de foco por CDP (`Emulation.setFocusEmulationEnabled`) y trae otra pestaña al frente, lo que produce un `blur` **real** del renderer; (4) `06-reset.spec.ts` acepta el aviso antes de comprobar el estado vacío, igual que `01-onboarding.spec.ts` (que prohíbe que el estado vacío esté en el DOM mientras el aviso bloquea). Se corrige además `popupError` para buscar la **acción** de §4.3 por la plantilla del mensaje: un `-32000 resetBlocked` mostraba la acción de «cuenta en uso por una dApp» |
...
#### 3.3.9 Definición de terminado (H3) — ✅ **COMPLETADO el 2026-09-11**
...
**Cierre verificado (2026-09-11).** Hito **COMPLETADO**. Batería real ejecutada con **Anvil en `127.0.0.1:8545` (chainId 31337)** y la dApp **autoservida** por el `webServer` de `playwright.config.ts` (D-H3-D); evidencia archivada en `RepoTecnico/evidencia/H3/` y acta `ACTA_H3.md`:
...
**Criterios del hito (§3.3.6) — todos CUMPLIDOS:** `CA-RF-13`, `CA-RF-14`/`CA-RF-45`, `CA-RF-15` (**cerrado tras D-H3-A**), `CA-RF-16`/`CA-RF-36`, `CA-RF-17`/`CA-RF-25`, `CA-RF-18`, `CA-RF-24` (parte `accountsChanged`), `CA-RF-26`, `CA-RF-27` y RNF-10/RNF-11; el detalle con su evidencia está en **`ACTA_H3.md` §3**.

**Evidencia por flujo (§7.4.1.f):** `07-provider-2026-09-11.json`, `08-eventos-2026-09-11.json`, `08-cambio-cuenta-2026-09-11.json`, `09-conectar-2026-09-11.{json,png}`, `13-revocar-2026-09-11.json`, `14-polling-2026-09-11.json`, `rpc-caido-2026-09-11.log` (spec `27`), `28-iframe-hostil-2026-09-11.json` y `cast-2026-09-11.log`.

#### 3.3.10 Cierre de H3: desviaciones aceptadas

Cinco puntos se registraron al poner en verde la batería de H3: **dos defectos del producto corregidos** (D-H3-A y D-H3-C), **una ambigüedad de contrato cerrada** por decisión (D-H3-B), **un defecto del arnés** (D-H3-D) y **una lección operativa del entorno** (nota de Anvil). **Ninguno rebaja un criterio de aceptación**: `CA-RF-15` pasa de **NO CUMPLIDO (mitad)** a **CUMPLIDO** con D-H3-A. Quedan registrados como **DEC-57..DEC-61** en `estado_proyecto.md`.

| # | Punto | Motivo real (medido en H3) | Impacto y corrección |
|---|---|---|---|
| **D-H3-A** | El cambio de cuenta activa desde el popup **no** emitía `accountsChanged` | `handleSetCurrentAccount` (`src/background/rpc/catalog.ts`) solo persistía la referencia y `emitAccountsChanged` (`src/background/events.ts`) **no tenía ningún emisor de producción** ligado al cambio de cuenta. Medido en `diagnostico-ca-rf-15-2026-09-11.log`: `wallet_setCurrentAccount` → `{"result":{"currentAccountRef":"idx:1"}}`, `accountsChanged recibidos tras el cambio: []` y `eth_accounts` seguía devolviendo la cuenta 0; `CA-RF-15` quedaba a medias | `handleSetCurrentAccount` emite `accountsChanged` con la **cuenta nueva** a las pestañas afectadas (`setCurrentAccount.spec.ts`). **`CA-RF-15` pasa a CUMPLIDO** (evidencia: `08-cambio-cuenta-2026-09-11.json`, `08-eventos-2026-09-11.json`) |
| **D-H3-B** | Ambigüedad de contrato: ¿la sesión por origen sigue a la cuenta activa? | Tras `wallet_setCurrentAccount { ref: 'idx:1' }`, la sesión vigente seguía autorizando `idx:0` (`pageMethods.ts` devolvía `session.account`): coherente con §3.2, pero hacía indecidible el «cambio desde el popup» de `CA-RF-15` sin fijar antes la semántica | Se adopta la **opción (a)**, equivalente a MetaMask: mientras la sesión siga vigente, la dApp comparte la **cuenta activa**; el cambio de cuenta **actualiza la cuenta compartida de las sesiones vigentes** y emite `accountsChanged` **sin borrar la sesión**. La **alternativa (b)** —sesión fijada a la cuenta elegida en la conexión— queda **descartada** y así se registra para que no se reintroduzca |
| **D-H3-C** | `senderGuard.ts` capturaba `INTERNAL_METHODS` al evaluar el módulo | Dentro de la cadena circular `catalog → connections → sessions → senderGuard → catalog`, el orden de importación podía evaluar `senderGuard` antes que la lista: `TypeError: Cannot read properties of undefined (reading 'includes')` y, con él, un **`-32603` en el router para cualquier petición** (evidencia: `diagnostico-import-order-2026-09-11.log`) | La lista se extrae a **`src/background/rpc/internalMethods.ts`** (módulo hoja, sin dependencias) y `senderGuard` la consume **sin depender del orden de importación**; el defecto deja de ser una fragilidad latente y `importOrder.spec.ts` lo fija (16 métodos) |
| **D-H3-D** | **Defecto del ARNÉS, no del producto:** 11 E2E fallaban con `page.goto: net::ERR_CONNECTION_REFUSED at http://localhost:5174/test.html` | El servidor de la dApp (`npm run dev`, puerto 5174) se **arrancaba a mano**: si nadie lo levantaba antes de `npm run test:e2e`, los flujos que abren `test.html` fallaban por un motivo ajeno al producto (**26 passed / 11 failed**) | Se añade **`webServer`** a `playwright.config.ts` (`command: 'npm run dev'`, `url: 'http://localhost:5174/test.html'`, `reuseExistingServer: true`, `timeout: 120 s`, `stdout: 'ignore'`): la suite **se autosirve la dApp**, espera a que responda, la reutiliza si ya estaba y la apaga al terminar. Resultado: **26 passed / 11 failed → 37 passed / 0 failed**. **Lección vinculante:** la suite **no** debe depender de que alguien arranque la dApp a mano; todo flujo que abra `test.html` cuenta con el `webServer` |
| **Anvil (nota operativa, no es defecto)** | El comando de arranque del arnés usaba **dos banderas problemáticas** | (1) **`--http.corsdomain` NO existe** en Anvil 1.7.2-dev: el proceso muere con `error: unexpected argument '--http.corsdomain' found` y toda la suite E2E falla después con `ERR_CONNECTION_REFUSED` / `error sending request`; (2) **`--silent`** no aparece en `anvil --help` y, con redirección de salida, el arranque es frágil (exit 1) | El comando que **sí** funciona en el arnés (medido en H4, 2026-09-12) es **`anvil --host 127.0.0.1 --port 8545 --chain-id 31337 --allow-origin "*"`**, **sin `--silent`**; `--allow-origin` admite **lista separada por comas** y **no** puede repetirse. Queda registrado en `entornos_globales.md` **v2.2** §2.1 con **los dos errores**, junto a la recomendación de que el **`global-setup` aborte con un mensaje claro si Anvil no responde** |
...
#### 3.4.9 Definición de terminado (H4) — ✅ **COMPLETADO el 2026-09-12**
...
**Cierre verificado (2026-09-12).** Hito **COMPLETADO**. Batería real ejecutada con **Anvil en `127.0.0.1:8545` (chainId 31337)** y la dApp **autoservida** por el `webServer` de `playwright.config.ts`; evidencia archivada en `RepoTecnico/evidencia/H4/` y acta `ACTA_H4.md`:

| Comando | Resultado real (2026-09-12) | Evidencia |
|---|---|---|
| `npx tsc -b` | **exit 0** — strict, 0 errores (incluye `src/`, `test/` y `e2e/`) | `evidencia/H4/typecheck-2026-09-12.log` (0 bytes = sin salida) |
...
| `npm run test` (Vitest) | **exit 0** — **587 passed / 41 ficheros, 0 failed** (los 403 de H1–H3 siguen verdes) | `evidencia/H4/vitest-2026-09-12.log` |
...
| Prueba real de extremo a extremo (sonda propia, fuera de `e2e/**`) | **OK** — 1 ETH, **UNA** ventana con preview decodificada y contador, hash `0x21f97614e3117d6e9083ba46b8b2be20c428008e5f6801adcc19fc00741833a6`, recibo **`status 0x1`**, cola vacía, `truekeate_inflight_tx` en `phase: 'broadcast'` (cuenta liberada) | `evidencia/H4/extremo-a-extremo-2026-09-12.{json,png}` |
| Suspensión REAL del SW por CDP a mitad de aprobación | **OK** — la alarma despierta al SW, la cola queda sin huérfanas, la dApp recibe **`4001`** («plazo establecido»), `nonce` sin avanzar (0 doble difusión) y reconstrucción **< 1 s** | `evidencia/H4/29-sw-suspendido-2026-09-12.{json,log}` |
| Concurrencia de 2 orígenes | **OK** — 2 `pending` coexistentes en UNA sola ventana con contador «2 solicitudes en espera» y las dos firmas obtenidas de esa misma ventana | `evidencia/H4/18-concurrencia-2026-09-12.json` |

**Criterios del hito (§3.4.6) — los 13 CUMPLIDOS:** `CA-RF-08`, `CA-RF-19`, `CA-RF-20`, `CA-RF-21`, `CA-RF-35`, `CA-RF-37`, `CA-RF-41`, `CA-RF-42`/`CA-RF-43`, `CA-RF-11` (parte 2), RNF-08, RNF-25 y `CA-RT-11`; `CA-RF-35`, `CA-RF-41` (parte E2E) y RNF-08 (parte E2E) pasan de **PARCIAL** a **CUMPLIDO** con `18-concurrencia` y `29-sw-suspendido` en verde. El detalle con su evidencia está en **`ACTA_H4.md` §4**.

#### 3.4.10 Estado del cierre de H4: cableado, desviaciones y bloqueos

**Batería real (2026-09-12, `TK_EVIDENCE_PHASE=H4`) — cierre definitivo del hito.**

| Comando | Resultado real | Evidencia |
|---|---|---|
| `npx tsc -b` | **exit 0** — 0 errores | `evidencia/H4/typecheck-2026-09-12.log` |
| `npm run build` | **exit 0** — 6 entradas + `dist/manifest.json` | `evidencia/H4/build-2026-09-12.log` |
| `npm run lint:prohibited` | **exit 0** — 0 hallazgos (127 ficheros de `src/`) | `evidencia/H4/lint-prohibited-2026-09-12.log` |
| `npm run test` (Vitest) | **exit 0** — **587 passed / 41 ficheros** | `evidencia/H4/vitest-2026-09-12.log` |
| `npm run test:e2e` (Playwright) | **exit 0** — **44 passed / 0 failed / 0 skipped** | `evidencia/H4/e2e-2026-09-12.{log,json}` |
| `npm run forge:test` | **exit 0** — **10 passed / 0 failed** | `evidencia/H4/forge-2026-09-12.log` |
| Prueba real de extremo a extremo | **OK** — 1 ETH, **UNA** ventana con preview decodificada y contador, hash `0x21f97614e3117d6e9083ba46b8b2be20c428008e5f6801adcc19fc00741833a6`, recibo **`status 0x1`**, cola vacía, `truekeate_inflight_tx` en `phase: 'broadcast'` (cuenta liberada) | `evidencia/H4/extremo-a-extremo-2026-09-12.{json,png}` |

**La ruta de los 6 métodos aprobables (`documento_tecnico.md` §3.1, paso a paso).**
`rpc/router.ts` → paso 6 `entry.requiresApproval && !isExtensionContext` → `deps.approve` → **`approvals/dispatch.ts`**:
(a) `resolveApprovalOrigin` (cuenta de la sesión vigente, `4100` sin sesión) → (b) para `eth_sendTransaction`,
`estimateGasOrBlock` **antes** de abrir ventana (`-32000` si falla) → (c) `buildApprovalPreview` (M19) → (d)
`enqueueApprovalRequest` (M14: RMW, cardinalidad 8/1/6, cota de 64 KiB, ventana de tasa) → (e) `armExpiryAlarm`
(M15, `chrome.alarms`) → (f) `showOldestPending` (M18, ventana global única) → (g) espera de la decisión por la
promesa de la cola (`decisions.ts`, resuelta por `SIGN_RESPONSE`, por la X de la ventana o por la alarma) → (h)
efecto: `nonce` recalculado (`getTransactionCount(account,'pending')`) + `getFeeData()`, firma tipo 2 (M11
`sign.ts`), difusión (M7 `txContract.ts`) **devolviendo el hash al difundir** y marca a `phase: 'broadcast'`
(cuenta liberada); para `personal_sign`/`eth_signTypedData_v4`, la firma `0x`+130 hex; rechazo o vencimiento →
`4001` con el literal de `diccionario_datos.md` §4.3 → (i) entrega con `deliverApprovalResolution` (M17) por
puerto o por pestaña/frame.

| # | Punto | Motivo real (medido) | Impacto y corrección |
|---|---|---|---|
| **D-H4-E1** | **Bloqueante corregido**: ningún método aprobable se encolaba y la ventana **nunca se abría** (`waitForEvent("page") Timeout 20000 ms` en `04-enviar`, `10-aprobar-tx`, `11-firmar-mensaje`) | El despacho de M19.b **ya existía** en `rpc/router.ts` (paso 6 → `defaultRouterDeps.approve` → `approvals/dispatch.ts`), pero era **inalcanzable**: `getCatalogEntry` filtra por `isSupportedMethod` y los 5 aprobables restantes seguían con `implemented: false` y **fuera** de `supportedMethods`, de modo que el router respondía `4200` en el paso 3 **antes** del despacho. `enqueueApprovalRequest` no tenía llamador porque el despacho nunca se ejecutaba | Los 6 aprobables pasan a `implemented: true` y `supportedMethods` incluye `PAGE_APPROVAL_METHODS` (`rpc/catalog.ts`); la guarda de contexto de `rpc/router.ts` deja pasar `wallet_revokePermissions` desde una **página** al despacho de aprobaciones y conserva el manejador interno para el popup. `messaging.spec.ts` actualiza las **dos** afirmaciones de H3 que H4 sustituye |
| **D-H4-E2** | **Regresión de H3 reportada**: `08-eventos.spec.ts:73`, `08-eventos.spec.ts:162` y `09-conectar.spec.ts:77` fallaban con «el flujo de conexión falló: `4001`» | **No reproducible como defecto de producción**: las tres pruebas están **verdes** en esta batería (aisladas y en la suite completa, 3 ejecuciones). El síntoma corresponde al plazo de conexión **inyectado por el arnés** (`VITE_CONNECT_TIMEOUT_MS=2000`, `e2e/global-setup.ts:47`): con el equipo cargado, `connect.html` puede tardar más de 2 s en pintar sus 5 filas con saldo real antes del clic | **Ninguna corrección de producción** (no hay defecto). No se desactiva ni se salta ninguna prueba. Si reaparece, la palanca es el plazo inyectado del arnés, **no** el orden de guardas ni el cálculo de `origin`/`sender` |
| **D-H4-E3** | **Defecto del ARNÉS**: `--http.corsdomain` **no existe** en Anvil 1.7.2-dev | Medido: `error: unexpected argument '--http.corsdomain' found` (el proceso muere y toda la suite posterior se queda sin nodo → `ERR_CONNECTION_REFUSED`) | `entornos_globales.md` **v2.2** §2.1 fija el comando verificado **`anvil --host 127.0.0.1 --port 8545 --chain-id 31337 --allow-origin "*"`**, **sin `--silent`**, y documenta **los dos errores** (`--http.corsdomain` inexistente; `--silent` fuera de `--help` y con arranque frágil). `--allow-origin` admite **lista separada por comas** y **no** puede repetirse |
| **D-H4-E5** | `isNotificationTabUrl` comparaba solo el `pathname` | Una página de la dApp en `/notification.html` se confundiría con la ventana única | **Ya corregido** en `approvals/focus.ts` (exige `chrome-extension:` + `host === runtime.id`) y fijado por `windowRediscovery.spec.ts:91` |
| **D-H4-E6** | La traza de difusión interrumpida no rellenaba `LogEntry.txHash` | El hash viajaba solo en `data.txHash`, que no es lo que leerá el panel de actividad de H5 (RF-31) | **Ya corregido** en `approvals/reconcile.ts` (campo de primer nivel cuando el dato existe) y fijado por `approvalReconcile.spec.ts:319` |
| **D-H4-E7** | `10-aprobar-tx` fallaba por *strict mode violation* en `.tk-section` con «Llamada a contrato» | `notification/TxPreviewPanel.tsx` **anidaba** un `div.tk-section` dentro de la sección del panel con el mismo texto de título | El bloque usa la utilidad `tk-view`: UNA sola `tk-section` identificable por su título, sin CSS nuevo |
| **D-H4-E8** | `11-firmar-eip712` no veía la dirección del contrato verificador | `notification/TypedDataPanel.tsx` pintaba `shortHex(verifyingContract)`: la dirección completa solo estaba en el `title`, y `CA-RF-20` exige mostrarla **en claro** | Se pinta la dirección **completa** (con el `title` conservado) |
| **D-H4-E9** | **CERRADO (2026-09-12)** — `18-concurrencia` rojo con `page.goto: net::ERR_CONNECTION_REFUSED at http://127.0.0.1:5174/test.html` | La prueba necesita **dos orígenes del mismo servidor** (`localhost` y `127.0.0.1`), pero el servidor de la dApp escuchaba **solo en IPv6**: `vite.config.ts` declaraba `server: { port: 5174, strictPort: true }` **sin `host`** y el defecto de Vite (`localhost`) resuelve a `::1` en este equipo (medido: `netstat` → `::1:5174`; `http://127.0.0.1:5174/test.html` → conexión rechazada) | **Corregido** en **`vite.config.ts`**: `server: { host: '127.0.0.1', port: 5174, strictPort: true }` (y el mismo `host` en `preview`). La dApp responde ya en `http://127.0.0.1:5174/test.html`, así que la prueba abre sus **dos orígenes** contra el MISMO servidor. **Verificado** por el E2E `18` en verde |
| **D-H4-E10** | **CERRADO (2026-09-12)** — `29-sw-suspendido` rojo: la dApp recibía `-32603` en lugar de `4001`, y con ello la promesa de la página se resolvía **antes** de que la alarma despertara al SW | (1) Al suspender el SW por CDP se cierra el canal `chrome.runtime.sendMessage` del relay y `content-script.ts` (`buildResponse` → `internalTransportError()`) respondía **`-32603`** ~130 ms después de la suspensión. (2) El `4001` de la reconciliación **sí** se empujaba (`reconcile.ts` → `deliverApprovalResolution`), pero con `id = approvalId`, que **no correlaciona** con el `id` que genera la capa inject: el `TRUEKEATE_RPC` del relay **no lo transportaba** y `lastApprovalId` estaba declarado y **nunca asignado** | **Corregido** en tres piezas: (a) **`src/content-script.ts`** ya **no** resuelve la petición con `-32603` al cerrarse el canal —reintenta solo cuando el fallo garantiza que el mensaje no llegó a nadie (`Could not establish connection. Receiving end does not exist.`) y, en cualquier otro fallo de transporte, **espera** la resolución empujada hasta agotar `SIGN_TIMEOUT_MS + TIMEOUT_SAFETY_MARGIN_MS`—; (b) la **correlación del salto 1** viaja como `requestId` en el sobre `TRUEKEATE_RPC` (`protocol.ts`), el router la propaga (`rpc/router.ts` → `approvals/dispatch.ts` → `approvals/queue.ts`) y se **persiste** con la entrada (`PendingRequest.requestId`), de modo que `deliverApprovalResolution` responde con el `id` de la página (`responseCorrelationId`) y **no** con `approvalId`, sobreviviendo a la suspensión; (c) `lastApprovalId` se asigna al **observar** el `approvalId` que acompaña a la resolución empujada, sin transformar el sobre (H-39). **Verificado** por el E2E `29` en verde (`4001` «plazo establecido», cola sin huérfanas, 0 doble difusión, reconstrucción < 1 s) |
| **D-H4-E11** | **CERRADO (2026-09-12)** — defecto medido al poner `18-concurrencia` en verde: el cuerpo de la SIGUIENTE solicitud **nunca** llegaba a la ventana única, que se quedaba con la anterior y se cerraba sola | El empuje iba por **`chrome.tabs.sendMessage(tabId, …)`** y `notification.html` es una **página de la extensión**, no un content script: medido con una sonda propia (Playwright + `dist/`), `chrome.tabs.sendMessage` a una pestaña con una página `chrome-extension://` responde **`Could not establish connection. Receiving end does not exist.`**. Además el `tabId` era irresoluble: sin el permiso `tabs` (retirado en H-36) `windows.getAll({ populate: true })` devuelve `url: null`, así que `notificationTabId` no encontraba la pestaña y `pushApprovalRequest` salía por `tabId === null` | **Corregido** en `approvals/ports.ts`: el empuje usa **`chrome.runtime.sendMessage`**, el único canal que alcanza a las páginas de la extensión (verificado: la ventana lo recibe), conservando `tabs.sendMessage` como respaldo para receptores que sí sean content scripts. **RNF-09 queda a salvo y medido**: un `chrome.runtime.sendMessage` emitido por el SW **no** llega a los content scripts (la dApp no recibe nada), así que el cuerpo no se difunde fuera de los contextos confiables. **Verificado** por el E2E `18` en verde (contador «1 solicitud en espera» tras aprobar la primera y segunda firma obtenida de la MISMA ventana) |
| **D-H4-E12** | **CERRADO (2026-09-12)** — carrera de orden medida en el mismo flujo: la ventana mostraba la siguiente solicitud pero su botón «Aprobar» quedaba **deshabilitado** y la cola se atascaba | `handleSignResponse` re-renderizaba la ventana (y empujaba la siguiente solicitud) **dentro** del mismo ciclo que la respuesta a `notification.html`, de modo que el empuje llegaba **antes** de que la ventana procesara su propio `SIGN_RESPONSE`: el `setDelivery` del empuje quedaba después pisado por el `setOutcome('approved')`/`decided` de la decisión recién contestada (medido: `Aprobar` deshabilitado con la solicitud `pending` correcta en la ventana y `truekeate_approval_window.shownApprovalId` ya apuntando a ella) | **Corregido**: `background.ts` responde `SIGN_RESPONSE` **primero** y hace la pasada de M18 **después** (`refreshApprovalWindowAfterDecision` → `showOldestPending({ repush: true })`), con `responses.ts` aceptando `refresh: false` y `focus.ts` re-`entregando` el cuerpo (idempotente) cuando la ventana ya mostraba esa solicitud. **Verificado** por el E2E `18` en verde |

**Criterios de H4:** **los 13 CUMPLIDOS** — `CA-RF-08`, `CA-RF-19`, `CA-RF-20`, `CA-RF-21`, `CA-RF-35`, `CA-RF-37`, `CA-RF-41`, `CA-RF-42`, `CA-RF-43`, `CA-RF-11` (parte 2), RNF-08, RNF-25 y `CA-RT-11`; `CA-RF-35`, `CA-RF-41` (parte E2E) y RNF-08 (parte E2E) pasan de **PARCIAL** a **CUMPLIDO** con `18-concurrencia` y `29-sw-suspendido` en verde. El detalle con su evidencia está en **`ACTA_H4.md` §4**.

**Conflicto de spec (verificado).** `queue.spec.ts:468-480` **ya** exige `-32000` (`inflightTxInProgress`) para la 2.ª firma de la misma cuenta, que es lo que manda `diccionario_datos.md` §4.3 v1.10: el código cumple y **no** hubo que ajustar ese spec. Los dos únicos `*.spec.ts` tocados son de `messaging.spec.ts`, porque codificaban la afirmación de H3 «los 6 aprobables responden `4200`» que H4 **debe** sustituir (ahora fijan el despacho a M19.b, con la línea de motivo). En el cierre de H4 se ajusta **una línea** de `e2e/18-concurrencia.spec.ts`: contaba las ventanas de decisión de forma **síncrona** antes de esperar su apertura, que es **asíncrona y posterior** a la escritura de la cola (§2.14: `enqueue` → `showOldestPending` → `chrome.windows.create`); ahora espera la condición observable (`esperarVentanaDeDecision`) y **después** cuenta —la expectativa del corpus («exactamente 1 `notification.html`») **no** cambia y **no** se relaja ninguna aserción—.
...
| H3 ✅ **(cerrado 2026-09-11)** | Provider, conexión, lectura, eventos, sesiones | 20 | 5 | 60 h |
| H4 ✅ **(cerrado 2026-09-12)** | Firma, cola persistida, ventana global, transacciones | 28 | 7 | 88 h |
...
| **H4 — ✅ COMPLETADO (2026-09-12)** | **H4: 9 RF Must** (RF-08, RF-19, RF-20, RF-21, RF-35, RF-37, RF-41, RF-42, RF-43) **+ guarda de RF-11** | CU-11..CU-16, CU-27, CU-28, CU-30 (guarda) | M7, M11, M14, M15, M16, M17, M18, M19, M42, M50..M54, M66, M33 (`truekeate_inflight_tx`), `contracts/` | **Los 13 CUMPLIDOS:** `CA-RF-08`, `CA-RF-19`, `CA-RF-20`, `CA-RF-21`, `CA-RF-35`, `CA-RF-37`, `CA-RF-41`, `CA-RF-42`, `CA-RF-43`, `CA-RF-11` (parte 2), RNF-08, RNF-25, `CA-RT-11` | **Vitest 587 passed / 41 ficheros** · **E2E 44 passed / 0 failed / 0 skipped** (`04`, `10`, `11-firmar-eip712`, `11-firmar-mensaje`, `18-concurrencia` y `29-sw-suspendido` en verde; los 37 de H1–H3 siguen verdes) · `tsc -b`, `build`, `lint:prohibited` exit 0 · `forge:test` 10/10 · **prueba real de extremo a extremo**: hash `0x21f9…33a6`, recibo `status 0x1`, cola vacía, marca en vuelo liberada · evidencia en `RepoTecnico/evidencia/H4/` (`ACTA_H4.md`) (§3.4.9/§3.4.10) |
| **H3 — ✅ COMPLETADO (2026-09-11)** | **H3: 11 RF Must** (RF-13..RF-18, RF-25..RF-27, RF-36, RF-45) | CU-08, CU-10, CU-17..CU-22, CU-26, CU-31 | M2, M3, M4, M5, M6, M20, M21, M22, M23, M26, M27, M35..M38, M44, M47, M48, M49, M55, M56 · `test.html` (4 flujos) | `CA-RF-13`, `CA-RF-14`, `CA-RF-15`, `CA-RF-16`, `CA-RF-17`, `CA-RF-18`, `CA-RF-24` (parte `accountsChanged`), `CA-RF-25`, `CA-RF-26`, `CA-RF-27`, `CA-RF-36`, `CA-RF-45` | **Vitest 403 passed / 28 ficheros** · **E2E 37 passed / 0 failed** (`07`, `08`, `09`, `13`, `14`, `27`, `28`) · `tsc -b`, `build` y `lint:prohibited` exit 0 · `cast chain-id` 31337 · evidencia en `RepoTecnico/evidencia/H3/` (`ACTA_H3.md`) (§3.3.9) |
| **H2 — ✅ COMPLETADO (2026-09-11)** | **H2: 11 RF Must** (RF-01..RF-05, RF-07, RF-09..RF-11, RF-33, RF-50) | CU-01..CU-09, CU-30, CU-32 | M8, M9, M10, M12, M13, M28, M29, M33, M34, M39..M41, M46, M57..M63 · M3/M4/M6/M22 | `CA-RF-01`, `CA-RF-02`, `CA-RF-03`, `CA-RF-04`, `CA-RF-05`, `CA-RF-07`, `CA-RF-09`, `CA-RF-10`, `CA-RF-11` (parte 1 **y** la guarda `-32000`), `CA-RF-33`, `CA-RF-50` | **Vitest 290 passed / 18 ficheros** (cobertura `crypto/` **95,13 %** y `shared/validation/` **97,27 %**) · **E2E 23 passed / 0 failed** (`01`, `02`, `03`, `05`, `06`, `16`, `25`) · `tsc -b` y `lint:prohibited` exit 0 · evidencia en `RepoTecnico/evidencia/H2/` (`ACTA_H2.md`) (§3.2.9) |
...
> **Estado de la matriz (2026-09-12).** Las filas de **H1**, **H2**, **H3** y **H4** pasan a **✅ COMPLETADO**. **H2** cierra sus **11 RF Must** con **Vitest 290 passed / 18 ficheros** (cobertura `crypto/` **95,13 %** y `shared/validation/` **97,27 %**, umbral ≥ 80 % cumplido) y **Playwright 23 passed / 0 failed / 0 skipped** contra **Anvil 31337** con `dist/` reconstruido por el `global-setup`; `tsc -b`, `npm run build` y `lint:prohibited` en verde y `grep` de `chrome.storage` y de `ethers` en `src/popup` → 0 (§3.2.9, `ACTA_H2.md`). Las 5 correcciones aceptadas de H2 quedan en **§3.2.10** (**DEC-52..DEC-56**). **H3** cierra sus **11 RF Must** con **Vitest 403 passed / 28 ficheros** y **Playwright 37 passed / 0 failed** contra **Anvil 31337** con la dApp autoservida (`webServer`), `tsc -b`, `npm run build` y `lint:prohibited` en verde y `rg` de `chrome.storage` y de `from 'ethers'` en `src/popup`/`src/connect` → **0** (§3.3.9, `ACTA_H3.md`); sus 5 puntos de cierre quedan en **§3.3.10** (**DEC-57..DEC-61**). **H4** cierra sus **9 RF Must + la guarda de RF-11** con **Vitest 587 passed / 41 ficheros**, **Playwright 44 passed / 0 failed / 0 skipped** contra **Anvil 31337** (`cast chain-id` = 31337) y la dApp autoservida, `tsc -b`/`build`/`lint:prohibited` exit 0, `forge:test` **10/10** y la prueba real de extremo a extremo con recibo `status 0x1` (§3.4.9, `ACTA_H4.md`); sus desviaciones y los 4 defectos corregidos al cerrarla quedan en **§3.4.10**. **El hito en curso pasa a H5 — Redes y observabilidad.**
...
|---|---|---|---|
| **1.5** | 2026-09-12 | **Cierre de H4 y desviaciones aceptadas.** (a) **§3.4.9** pasa a **✅ COMPLETADO (2026-09-12)** con la **batería real de cierre**: `npx tsc -b` exit 0 (strict), `npm run build` exit 0 (6 entradas + `dist/manifest.json`), `npm run lint:prohibited` exit 0, **Vitest 587 passed / 41 ficheros**, **Playwright 44 passed / 0 failed / 0 skipped** (los 37 de H1–H3 siguen verdes) y `npm run forge:test` **10/10**, más la prueba real de extremo a extremo (hash `0x21f9…33a6`, recibo `status 0x1`, cola vacía, marca en vuelo liberada), la **suspensión real del SW por CDP** con `4001` y reconstrucción **< 1 s** y la **concurrencia de 2 orígenes** en UNA sola ventana con contador «2». (b) **§3.4.10** cierra los dos bloqueos y añade los dos defectos medidos al poner `18-concurrencia` en verde: **D-H4-E9** (`vite.config.ts` con `server.host: '127.0.0.1'`: la dApp responde en IPv4 y la prueba puede usar sus dos orígenes), **D-H4-E10** (el relay **no** responde `-32603` al cerrarse el canal —reintenta solo si el mensaje no llegó a nadie y, si no, espera hasta `SIGN_TIMEOUT_MS + TIMEOUT_SAFETY_MARGIN_MS`— y la **correlación del salto 1** viaja como `requestId` persistido, de modo que `deliverApprovalResolution` responde con el `id` de la página), **D-H4-E11** (el empuje del cuerpo a la ventana única pasa a **`chrome.runtime.sendMessage`**: medido, `chrome.tabs.sendMessage` **no** alcanza a una página `chrome-extension://` y el `tabId` es irresoluble sin el permiso `tabs`; medido también que `runtime.sendMessage` **no** llega a los content scripts, con lo que **RNF-09 se mantiene**) y **D-H4-E12** (la pasada de M18 se hace **después** de responder `SIGN_RESPONSE`, con re-entrega idempotente del cuerpo, para que la ventana no quede con el botón «Aprobar» deshabilitado por la carrera de orden). (c) Se conservan íntegros los puntos de la v1.4: **D-H4-E1** (cableado inalcanzable por `implemented: false`), **D-H4-E2** (regresión de H3 **verificada en verde**, sin defecto de producción), **D-H4-E3** (Anvil `--allow-origin "*"`, no `--http.corsdomain`, **sin `--silent`**), **D-H4-E5**/**D-H4-E6** (ya corregidos), **D-H4-E7**/**D-H4-E8** (defectos de las ventanas de la UI). (d) **§5 y §6** marcan **H4 ✅ COMPLETADO**; **§6** añade la fila de la matriz del hito con sus 13 criterios **CUMPLIDOS**. Decisiones **DEC-62..DEC-68** de `estado_proyecto.md` v2.4 (con **DEC-65 cerrada**) y acta `ACTA_H4.md`. | `estado_proyecto.md` **v2.4** · `evidencia/H4/ACTA_H4.md` · `entornos_globales.md` v2.2 · `vite.config.ts` · `src/content-script.ts` · `src/background/approvals/**` |
| **1.4** | 2026-09-12 | **Cableado de H4 en producción y estado REAL del hito (EN CURSO, no completado).** (a) **§3.4.9** conserva su definición de terminado y añade el **estado real**: los 6 métodos aprobables **ya se despachan** (preview M19 → cola M14 → plazo M15 → ventana única M18 → decisión → firma M11 → difusión M7 con **hash al difundir** → entrega M17) y la prueba de extremo a extremo es verde, pero la suite cierra en **42 passed / 2 failed**, así que **H4 NO se marca completado** y el hito actual sigue siendo H4. (b) **Nueva §3.4.10 «Estado del cierre de H4: cableado, desviaciones y bloqueos»** con la **batería real** (`tsc -b`, `build` y `lint:prohibited` exit 0, **Vitest 587 passed / 41 ficheros**, **Playwright 42 passed / 2 failed**, `forge:test` **10/10** y la prueba real de extremo a extremo: 1 ETH, UNA ventana con preview decodificada y contador, hash `0x21f97614e3117d6e9083ba46b8b2be20c428008e5f6801adcc19fc00741833a6`, recibo `status 0x1`, cola vacía y marca en vuelo liberada) y con la ruta **paso a paso** de los 6 aprobables; puntos registrados: **D-H4-E1** (la causa del bloqueo eran los **metadatos del catálogo** —`implemented: false` y fuera de `supportedMethods`—, no la falta de despacho: el router respondía `4200` antes de llegar a `approvals/dispatch.ts`), **D-H4-E2** (la regresión de H3 está **verificada en verde**; el síntoma `4001` es el plazo de conexión **inyectado** por el arnés —`VITE_CONNECT_TIMEOUT_MS=2000`—, **sin** defecto de producción y **sin** desactivar ninguna prueba), **D-H4-E3** (el comando de Anvil del arnés pasa a **`--allow-origin "*"`**, **sin `--silent`**), **D-H4-E5** y **D-H4-E6** (ya corregidos), **D-H4-E7**/**D-H4-E8** (defectos de las ventanas de la UI) y los **dos bloqueos** **D-H4-E9**/**D-H4-E10** con su corrección exacta, pendiente entonces de autorización. | `estado_proyecto.md` v2.3 · `entornos_globales.md` v2.2 |
| **1.3** | 2026-09-11 | **Cierre de H3 y desviaciones aceptadas.** (a) **§3.3.9** pasa a **✅ COMPLETADO (2026-09-11)** con la **batería real**: `npx tsc -b` exit 0 (strict), `npm run build` exit 0 (6 entradas + `dist/manifest.json`), `npm run lint:prohibited` exit 0, **Vitest 403 passed / 28 ficheros**, **Playwright 37 passed / 0 failed** con Anvil 31337 y la dApp **autoservida** por el `webServer`, `rg -n "chrome\.storage"` y `rg -n "from 'ethers'"` en `src/popup`/`src/connect` → **0 coincidencias**, y `cast chain-id` = **31337**, `cast block-number` **≥ 1** y `cast balance 0xf39F…2266` = **10 000 ETH** (evidencia en `RepoTecnico/evidencia/H3/` y `ACTA_H3.md`). (b) **Nueva §3.3.10 «Cierre de H3: desviaciones aceptadas»** con los **5 puntos** y su impacto: **D-H3-A** el cambio de cuenta activa desde el popup **no emitía `accountsChanged`** (`emitAccountsChanged` sin emisor de producción), corregido en `handleSetCurrentAccount` (`src/background/rpc/catalog.ts`), con lo que **`CA-RF-15` pasa a CUMPLIDO**; **D-H3-B** se fija la **semántica de la sesión por origen** en la **opción (a)** —el cambio de cuenta actualiza la **cuenta compartida** de las sesiones vigentes y emite `accountsChanged`— y se **descarta** la opción (b) (sesión fijada en la conexión); **D-H3-C** `senderGuard` capturaba `INTERNAL_METHODS` al evaluar el módulo (cadena circular → `TypeError` y `-32603` en el router) y la lista se extrae a **`src/background/rpc/internalMethods.ts`**; **D-H3-D** **defecto del arnés**: 11 E2E fallaban con `net::ERR_CONNECTION_REFUSED` en `http://localhost:5174/test.html` porque la dApp se arrancaba a mano — se añade **`webServer`** a `playwright.config.ts` (**26 passed / 11 failed → 37 passed / 0 failed**) y queda la **lección vinculante** de que la suite **no** debe depender de que alguien arranque la dApp; **nota operativa de Anvil**: `anvil --silent` **no arranca** con `Start-Process` y redirección de salida (exit 1 → `ERR_CONNECTION_REFUSED`), el comando válido es `anvil --host 127.0.0.1 --port 8545 --chain-id 31337 --http.corsdomain "*"` **sin** `--silent` (registrado en `entornos_globales.md` §2.1) y se recomienda que el **`global-setup` aborte con un mensaje claro si Anvil no responde**. (c) **§2, §5 y §6** marcan **H3 ✅ COMPLETADO** (fecha y batería) y la matriz de §6 gana su **fila resumen de H3**; el hito en curso pasa a **H4**. Decisiones **DEC-57..DEC-61** de `estado_proyecto.md` v2.2. | `estado_proyecto.md` v2.2 · `documento_tecnico.md` v1.7 · `entornos_globales.md` v2.1 · `ACTA_H3.md` · `RepoTecnico/evidencia/H3/` |
| **1.2** | 2026-09-11 | **Cierre de H2 y correcciones aceptadas.** (a) **§3.2.9** pasa a **✅ COMPLETADO (2026-09-11)** con la **batería real**: `npx tsc -b` exit 0 (strict), `npm run build` exit 0 (6 entradas + `dist/manifest.json`), `npm run lint:prohibited` exit 0, **Vitest 290 passed / 18 ficheros**, `npm run coverage` exit 0 (`crypto/` **95,13 %** y `shared/validation/` **97,27 %**, umbral ≥ 80 % cumplido), **Playwright 23 passed / 0 failed / 0 skipped / 0 flaky** con Anvil 31337, y `rg -n "chrome\.storage"` y `rg -n "from 'ethers'"` en `src/popup` → **0 coincidencias** (evidencia en `RepoTecnico/evidencia/H2/` y `ACTA_H2.md`). (b) **Nueva §3.2.10 «Cierre de H2: desviaciones aceptadas»** con las **5 correcciones** y su impacto: **D-H2-G** el revelado se autoriza por **origen + ruta del popup** y no por `sender.tab` (exigir `tabId === null` devolvía `4200` siempre y dejaba `CA-RF-50` inalcanzable; ahora la guarda de sesión vigente responde `-32000`); **D-H2-H** **orden de guardas** del reset en el SW (guardas → confirmación, CU-30) y **consulta previa** del popup con `wallet_resetWallet { confirm: false }`, que no abre el diálogo destructivo cuando hay `-32000`; **D-H2-I** el aviso «Cartera reseteada» se eleva a `App` porque el reset desmonta `SecurityView`; **D-H2-J** **borrado del portapapeles aplazado** al recuperar el foco (medido: con el documento sin foco Chrome responde `NotAllowedError: Document is not focused` y `execCommand('copy')` no modifica el portapapeles), guardando solo la huella SHA-256 (`clipboardHash` de §3.10) y sin borrar a ciegas; **D-H2-K** correcciones del **arnés E2E** (siembra coherente de `truekeate_current_account`/`truekeate_accounts`, selector acotado al `form`, `loseFocusToOtherPage` para provocar la pérdida de foco **real** en headless, aceptación del aviso RNF-23 tras el reset y acción de §4.3 por plantilla de mensaje en `popupError`). (c) **§2, §5 y §6** marcan **H2 ✅ COMPLETADO** y la fila de la matriz de §6 se añade con sus 11 RF y su evidencia. Decisiones **DEC-52..DEC-56** de `estado_proyecto.md` v2.1. | `estado_proyecto.md` v2.1 · `evidencia/H2/ACTA_H2.md` · `evidencia/H2/{typecheck,build,lint-prohibited,vitest,playwright,coverage,e2e}-2026-09-11.*` |
