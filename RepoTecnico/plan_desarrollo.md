...
> **Fase:** 3 — Desarrollo · **Versión:** 1.0 · **Estado:** ✅ propuesto para aprobación (arranque de la Fase 3)
...
5. **Puerta de comandos**: desde **H1** y en todos los hitos siguientes, `npm run test`, `npm run test:e2e`, `forge test --match-contract EIP712VerifierTest` y `npm run check:mermaid` terminan con **exit 0** (`CA-RT-07`, `documento_tecnico.md` §9).
...
| **Foundry (`forge`)** | Mismo rango `>=1.0.0 <2.0.0` | `forge --version` y `forge test --match-contract EIP712VerifierTest` | H1 (instrumento), H4 (correspondencia wallet↔contrato), H6 (cierre) |
...
|---|---|---|---|---|---|---|
| **H1** | Andamiaje, build MV3 de las 6 entradas y arnés de pruebas | **— (0 RF; habilita RT-01..RT-13)** | CU-22 (detección), CU-36 (build) | M1, M2, M3/M4 (esqueleto), M6, M20, M21, M33, M35, M36, M37, M38, M55, M56, M57, M64, M65, `contracts/` | `Comando:` build/tsc/lint · `Vitest: manifest.spec.ts`, `naming.spec.ts`, `windowContract.spec.ts`, `errors.spec.ts` · `E2E: 01-onboarding.spec.ts` (carga de `dist/`) · `forge test` | **16 h / 4 sesiones** |
...
| 1.17 | Instrumento Foundry del contrato verificador | `contracts/foundry.toml` (`solc` **fijado** `0.8.24`, `evm_version = "cancun"`, `optimizer = false`), `contracts/src/EIP712Verifier.sol`, `contracts/test/EIP712Verifier.t.sol`, `contracts/test/fixtures/eip712-signature.json` | `forge test --match-contract EIP712VerifierTest` exit 0 con los **5 casos obligatorios** (firma válida `true`; firmante incorrecto `false`; dominio alterado `false`; firma malformada `false` **sin revert**; byte alterado `false`); el fixture está versionado y no depende de claves aleatorias ni de red (`CA-RT-11`) |
...
- **Forge**: `forge test --match-contract EIP712VerifierTest` (5 casos).
...
#### 3.1.9 Definición de terminado (H1)
`npm ci && npm run build` exit 0 con las 6 entradas y `dist/manifest.json`; `npx tsc -b`, `npm run test`, `npm run test:e2e`, `forge test` y `npm run check:mermaid` en verde; `dist/` cargado en `chrome://extensions` sin errores, con icono en el toolbar, popup 380×600 en estado vacío y `test.html` detectando `window.truekeate`/`window.codecrypto`; acta y evidencia archivadas en `RepoTecnico/evidencia/H1/`; `estado_proyecto.md` actualizado.
...
- **Forge**: `forge test --match-contract EIP712VerifierTest` sobre el fixture y sobre la firma de la wallet.
...
- **Comandos**: `npm ci && npm run build` en Windows y en WSL2/CI; `npm run test -- --coverage`; `npm run check:mermaid`; `forge test --match-contract EIP712VerifierTest`; `git ls-files`.
...
|---|---|---|---|---|
| H1 | Andamiaje, build MV3, arnés, instrumento Forge | 16 | 4 | 16 h |
...
| — | **H1: 0 RF Must (habilitador)** | CU-22, CU-36 (parcial) | M1, M2, M20, M21, M33, M35..M38, M55..M57, M64, M65, `contracts/` | `CA-RT-01`..`CA-RT-13` | `Comando: npm ci && npm run build` · `Vitest: manifest.spec.ts` · `forge test` |
...
2. **Puerta de pruebas completa:** `npm run test`, `npm run test:e2e`, `forge test --match-contract EIP712VerifierTest` y `npm run check:mermaid` con **exit 0** (RT-07).
