...
> **Fase:** 3 — Desarrollo · **Versión:** 1.4 · **Estado:** 🚧 **EN CURSO — H1 ✅ COMPLETADO (2026-09-11) · H2 ✅ COMPLETADO (2026-09-11) · H3 ✅ COMPLETADO (2026-09-11)**; hito en curso: **H4 — Firma, aprobación y transacciones** (**🚧 cableado en producción y 42/44 E2E en verde; NO completado**: quedan dos pruebas rojas por causas ajenas al producto, §3.4.10)
> **Cambios de la v1.4 (resumen).** Se registra la **puesta en verde del cableado de H4** y su estado REAL: la ruta de los **6 métodos aprobables** queda operativa en producción (`rpc/catalog.ts` + `rpc/router.ts`: los 5 aprobables pasan a `implemented: true` y `supportedMethods`, y su despacho llega a `approvals/dispatch.ts`), con **Vitest 587 passed / 41 ficheros**, **Playwright 42 passed / 2 failed**, `tsc -b`, `build`, `lint:prohibited` y `forge:test` (10/10) en verde, y una **prueba real de extremo a extremo** archivada (`evidencia/H4/extremo-a-extremo-2026-09-12.{json,png}`: 1 ETH, UNA ventana con preview decodificada y contador, hash `0x21f9…33a6`, recibo `status 0x1`, cola vacía y marca en vuelo liberada). Se abre la nueva **§3.4.10 «Estado del cierre de H4: cableado, desviaciones y bloqueos»** con los puntos medidos —corrección del cableado (**D-H4-E1**), regresión de H3 **verificada en verde** (**D-H4-E2**), las dos observaciones ya corregidas (**D-H4-E5**, **D-H4-E6**), dos defectos nuevos de las ventanas de la UI (**D-H4-E7**, **D-H4-E8**), la **corrección del comando de Anvil** del arnés (**D-H4-E3**, `--http.corsdomain` no existe en 1.7.2-dev) y los **dos bloqueos abiertos** (**D-H4-E9**, **D-H4-E10**) con su causa raíz y la corrección exacta que falta. **H4 no se marca completado**: `CA-RF-35`, `CA-RF-41` (parte E2E) y RNF-08 (parte E2E) siguen **PARCIAL**.
...
> **ESTADO REAL a 2026-09-12: 🚧 NO COMPLETADO.** El cableado de producción de los 6 métodos aprobables **ya está operativo** y la prueba de extremo a extremo del hito es **verde** (hash y recibo `status 1` observados), pero la definición de terminado exige **«E2E del hito en verde»** y la suite cierra en **42 passed / 2 failed**: `18-concurrencia` (CA-RF-35 parcial) y `29-sw-suspendido` (CA-RF-41/RNF-08 parciales). Ambas causas están **fuera del producto y fuera del terreno autorizado** (detalle en §3.4.10). **H4 no se marca ✅ COMPLETADO y el hito actual sigue siendo H4** (no se abre H5).

#### 3.4.10 Estado del cierre de H4: cableado, desviaciones y bloqueos

**Batería real (2026-09-12, `TK_EVIDENCE_PHASE=H4`).**
...
| `npm run test:e2e` (Playwright) | **exit 1** — **42 passed / 2 failed** | `evidencia/H4/e2e-2026-09-12.{log,json}` |
...
| **D-H4-E9** | **BLOQUEO ABIERTO** — `18-concurrencia` rojo: `page.goto: net::ERR_CONNECTION_REFUSED at http://127.0.0.1:5174/test.html` | La prueba necesita **dos orígenes del mismo servidor** (`localhost` y `127.0.0.1`), pero el servidor de la dApp escucha **solo en IPv6**: `vite.config.ts` declara `server: { port: 5174, strictPort: true }` **sin `host`**, y el defecto de Vite (`localhost`) resuelve a `::1` en este equipo (medido: `Get-NetTCPConnection -LocalPort 5174` → `::1`; `http://127.0.0.1:5174/test.html` → conexión rechazada) | Añadir `host: '127.0.0.1'` (o `host: true`) al bloque `server` de **`vite.config.ts`**. Es una **configuración**, expresamente fuera del terreno autorizado de esta sesión: **queda pendiente de autorización** |
| **D-H4-E10** | **BLOQUEO ABIERTO** — `29-sw-suspendido` rojo: la dApp recibe `-32603` en lugar de `4001` | Al suspender el SW por CDP se cierra el canal del content script y `src/content-script.ts:225` (`buildResponse` → `internalTransportError()`) responde a la página **`-32603`** ~130 ms después de la suspensión (traza: `Unchecked runtime.lastError: … the message channel closed before a response was received`), **antes** de que la alarma despierte al SW. El `4001` de la reconciliación sí se empuja (`reconcile.ts:392` → `deliverApprovalResolution`) pero con `id = approvalId`, que **no correlaciona** con el `id` de la capa inject (el `TRUEKEATE_RPC` del relay no lo transporta: `content-script.ts:266-277`; el estado `lastApprovalId` de `content-script.ts:311` está declarado y **nunca asignado**). Contradice H-07 («la capa inject/content delega **siempre** en el `approvalId`») | Que el relay **no** resuelva la petición de página con `internalTransportError()` al cerrarse el canal y que la entrega empujada se correlacione con la petición en vuelo: ambos cambios viven en **`src/content-script.ts`**, expresamente fuera del terreno autorizado de esta sesión: **queda pendiente de autorización** |

**Criterios de H4:** **CUMPLIDOS** `CA-RF-08`, `CA-RF-19`, `CA-RF-20`, `CA-RF-21`, `CA-RF-37`, `CA-RF-42`, `CA-RF-43`, `CA-RF-11` (parte 2), RNF-25 y `CA-RT-11`; **PARCIALES** `CA-RF-35`, `CA-RF-41` (parte E2E) y RNF-08 (parte E2E) por `D-H4-E9`/`D-H4-E10`. El detalle con su evidencia está en **`ACTA_H4.md` §4**.
...
| **H4 — 🚧 PARCIAL (2026-09-12)** | **H4: 9 RF Must** (RF-08, RF-19, RF-20, RF-21, RF-35, RF-37, RF-41, RF-42, RF-43) **+ guarda de RF-11** | CU-11..CU-16, CU-27, CU-28, CU-30 (guarda) | M7, M11, M14, M15, M16, M17, M18, M19, M42, M50..M54, M66, M33 (`truekeate_inflight_tx`), `contracts/` | **CUMPLIDOS:** `CA-RF-08`, `CA-RF-19`, `CA-RF-20`, `CA-RF-21`, `CA-RF-37`, `CA-RF-42`, `CA-RF-43`, `CA-RF-11` (parte 2), RNF-25, `CA-RT-11` · **PARCIALES:** `CA-RF-35`, `CA-RF-41` (parte E2E), RNF-08 (parte E2E) | **Vitest 587 passed / 41 ficheros** · **E2E 42 passed / 2 failed** (`04`, `10`, `11-firmar-eip712`, `11-firmar-mensaje` y las partes no ambientales de `18`/`29` en verde; rojas `18-concurrencia` y `29-sw-suspendido` por **D-H4-E9**/**D-H4-E10**, causas fuera del producto) · `tsc -b`, `build`, `lint:prohibited` exit 0 · `forge:test` 10/10 · **prueba real de extremo a extremo**: hash `0x21f9…33a6`, recibo `status 0x1`, cola vacía, marca en vuelo liberada · evidencia en `RepoTecnico/evidencia/H4/` (`ACTA_H4.md`) (§3.4.9/§3.4.10) |
...
|---|---|---|---|
| **1.4** | 2026-09-12 | **Cableado de H4 en producción y estado REAL del hito (EN CURSO, no completado).** (a) **§3.4.9** conserva su definición de terminado y añade el **estado real**: los 6 métodos aprobables **ya se despachan** (preview M19 → cola M14 → plazo M15 → ventana única M18 → decisión → firma M11 → difusión M7 con **hash al difundir** → entrega M17) y la prueba de extremo a extremo es verde, pero la suite cierra en **42 passed / 2 failed**, así que **H4 NO se marca completado** y el hito actual sigue siendo H4. (b) **Nueva §3.4.10 «Estado del cierre de H4: cableado, desviaciones y bloqueos»** con la **batería real** (`tsc -b`, `build` y `lint:prohibited` exit 0, **Vitest 587 passed / 41 ficheros**, **Playwright 42 passed / 2 failed**, `forge:test` **10/10** y la prueba real de extremo a extremo: 1 ETH, UNA ventana con preview decodificada y contador, hash `0x21f97614e3117d6e9083ba46b8b2be20c428008e5f6801adcc19fc00741833a6`, recibo `status 0x1`, cola vacía y marca en vuelo liberada) y con la ruta **paso a paso** de los 6 aprobables; puntos registrados: **D-H4-E1** (la causa del bloqueo eran los **metadatos del catálogo** —`implemented: false` y fuera de `supportedMethods`—, no la falta de despacho: el router respondía `4200` antes de llegar a `approvals/dispatch.ts`), **D-H4-E2** (la regresión de H3 está **verificada en verde** en las 3 ejecuciones; el síntoma `4001` es el plazo de conexión **inyectado** por el arnés —`VITE_CONNECT_TIMEOUT_MS=2000`— con el equipo cargado, **sin** defecto de producción y **sin** desactivar ninguna prueba), **D-H4-E3** (el comando de Anvil del arnés pasa a **`--allow-origin "*"`**, **sin `--silent`**, porque `--http.corsdomain` **no existe** en Anvil 1.7.2-dev), **D-H4-E5** y **D-H4-E6** (ya corregidos y fijados por `windowRediscovery.spec.ts` y `approvalReconcile.spec.ts`), **D-H4-E7** (se elimina la `tk-section` anidada de `TxPreviewPanel.tsx` que rompía la lectura del panel por su título) y **D-H4-E8** (`TypedDataPanel.tsx` pinta la dirección **completa** del contrato verificador, `CA-RF-20`), más los **dos bloqueos abiertos**: **D-H4-E9** (`18-concurrencia`: el servidor de la dApp escucha **solo en `[::1]`** porque `vite.config.ts` no declara `host`) y **D-H4-E10** (`29-sw-suspendido`: al suspender el SW, `src/content-script.ts:225` responde `-32603` a la página antes de que la alarma despierte al SW, y el `4001` empujado no correlaciona por `approvalId`), ambos **fuera del terreno autorizado** y **pendientes de autorización**. (c) Se corrigen las citas de la bandera de Anvil en **§3.3.10** y **§6**. (d) **§6** añade la fila **«H4 — 🚧 PARCIAL (2026-09-12)»** con los criterios cumplidos y parciales. Decisiones **DEC-62..DEC-65** de `estado_proyecto.md` v2.3. | `estado_proyecto.md` v2.3 · `entornos_globales.md` v2.2 · `ACTA_H4.md` · `evidencia/H4/extremo-a-extremo-2026-09-12.{json,png}` · `src/background/rpc/{catalog,router}.ts` · `src/notification/{TxPreviewPanel,TypedDataPanel}.tsx` |
