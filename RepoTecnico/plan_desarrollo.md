...
> **Fase:** 3 — Desarrollo · **Versión:** 1.2 · **Estado:** 🚧 **EN CURSO — H1 ✅ COMPLETADO (2026-09-11) · H2 ✅ COMPLETADO (2026-09-11)**; hito en curso: **H3 — Provider, conexión y lectura**
...
> **Estado de la matriz (2026-09-11).** Las filas de **H1** y **H2** pasan a **✅ COMPLETADO**. **H2** cierra sus **11 RF Must** con **Vitest 290 passed / 18 ficheros** (cobertura `crypto/` **95,13 %** y `shared/validation/` **97,27 %**, umbral ≥ 80 % cumplido) y **Playwright 23 passed / 0 failed / 0 skipped** contra **Anvil 31337** con `dist/` reconstruido por el `global-setup`; `tsc -b`, `npm run build` y `lint:prohibited` en verde y `grep` de `chrome.storage` y de `ethers` en `src/popup` → 0 (§3.2.9, `ACTA_H2.md`). Las 5 correcciones aceptadas de H2 quedan en **§3.2.10** (**DEC-52..DEC-56**). El hito en curso es **H3**; las 5 desviaciones aceptadas de H1 siguen en **§3.1.10** (**DEC-47..DEC-51**).
