...
> **Fase:** 3 — Desarrollo · **Versión:** 1.3 · **Estado:** 🚧 **EN CURSO — H1 ✅ COMPLETADO (2026-09-11) · H2 ✅ COMPLETADO (2026-09-11) · H3 ✅ COMPLETADO (2026-09-11)**; hito en curso: **H4 — Firma, aprobación y transacciones**
...
| **Anvil (nota operativa, no es defecto)** | `anvil --silent` **no arranca** cuando se lanza mediante `Start-Process` con redirección de salida | El proceso muere con **exit 1** y toda la suite E2E falla con `ERR_CONNECTION_REFUSED` / `error sending request`, un síntoma que se confunde con un defecto de la extensión | El comando que **sí** funciona en el arnés es `anvil --host 127.0.0.1 --port 8545 --chain-id 31337 --http.corsdomain "*"` **sin `--silent`**; queda registrado en `entornos_globales.md` §2.1 con el aviso, junto a la recomendación de que el **`global-setup` aborte con un mensaje claro si Anvil no responde** (hoy solo emite un aviso y las pruebas con red se saltan o fallan más tarde) |
