...
| RNF-09 | Seguridad | La clave privada y la frase semilla **nunca** salen del Service Worker ni se exponen a la página (no viajan por `window.postMessage`); tampoco se registran en logs los **payloads firmados completos** (mensaje de `personal_sign`, JSON de `eth_signTypedData_v4`, calldata) (H-42). | Inspección de mensajes en content-script + `Vitest: logRedaction.spec.ts` (0 coincidencias de mnemonic, clave o payload íntegro en los logs) |
...
**Política de redacción de `params` (H-42) — obligatoria:** nunca se registran claves privadas ni el mnemonic (ya prohibido en `diccionario_datos.md:184`) y **tampoco** el payload firmado íntegro: en `personal_sign` y `eth_signTypedData_v4` se guarda el **hash keccak256 del payload** y su longitud; en `eth_sendTransaction` se guardan `to`, `value`, `dataLength` y el **selector** (nunca el `data` completo). El log se exporta en JSON para diagnóstico y vive en `chrome.storage.local` gestionado por el Service Worker.
...
Verificación: test que lee `tokens.css` y calcula el ratio WCAG de cada par + `axe-core` dentro de la suite E2E (`E2E: 24-accesibilidad.spec.ts`, 0 violaciones críticas).
...
| RT-04 | Plataforma | Chrome Extension Manifest V3: Service Worker (`type: module`), Content Script, Inject Script, `chrome.storage.local`, `chrome.alarms`, `chrome.windows`, `chrome.notifications` (H-02). **Permisos mínimos (H-36):** `tabs`, `activeTab` y `scripting` solo se declaran si el código usa una capacidad que los exija; `host_permissions` solo para las redes dadas de alta. **Política de RPC arbitrario (H-36):** `wallet_addEthereumChain` valida esquema (`https` preferente; `http` solo en local) y host, exige aprobación explícita del usuario y solicita el permiso en runtime (`chrome.permissions.request`), registrándolo por red en `truekeate_networks`. | `manifest.json` declara `storage`, `alarms` y `notifications`; `tabs`/`activeTab`/`scripting` solo con uso demostrado; el alta de red sin permiso de host se rechaza con error tipado. | `Vitest: manifest.spec.ts` · `Revisión: dist/manifest.json` · `E2E: 12-redes.spec.ts — permiso en runtime` |
...
| RT-13 | Nomenclatura | Producto **TrueKeate Wallet**; provider `window.truekeate` **con alias `window.codecrypto`**; EIP-6963 `name: "TrueKeate"` y `rdns: "academy.codecrypto.truekeate"`; prefijo de storage `truekeate_`; tipos de mensaje `TRUEKEATE_REQUEST/RESPONSE/EVENT/RPC`; dominio EIP-712 de la dApp `TrueKeate Test App`. | `window.truekeate === window.codecrypto`, prefijo `truekeate_` y tipos `TRUEKEATE_*`; `rdns` = `academy.codecrypto.truekeate`. | `Vitest: naming.spec.ts` · `Revisión: grep de codecrypto_ en src/` |
...
```
**Evidencia:** `E2E: 11-firmar-eip712.spec.ts` · `Vitest: typedData.spec.ts` · `Forge: EIP712Verifier.t.sol`. **Cierra H-11b (EIP-712) y H-40.**
...
Cuando el usuario aprueba
Entonces la red aparece en la lista con ese chainId y pasa a ser la activa
Pero con un RPC no https fuera de local, la solicitud se rechaza con error tipado
```
**Evidencia:** `E2E: 12-redes.spec.ts` · `Vitest: networks.spec.ts`. **Cierra H-36 (política de RPC arbitrario).**
...
#### CA-RF-35 · `notification.html` anti-phishing
```gherkin
...
Pero si el mismo origen vuelve a solicitar mientras hay una pendiente, no se abre una segunda ventana
```
**Evidencia:** `E2E: 10-aprobar-tx.spec.ts — foco y origen`. **Cierra H-35.**
...
#### CA-RF-39 · Notificaciones de Chrome
```gherkin
Dado el permiso notifications concedido
...
```
**Evidencia:** `E2E: 20-notificaciones.spec.ts`.
...
**Evidencia:** `Comando: forge test --match-contract EIP712VerifierTest`.
