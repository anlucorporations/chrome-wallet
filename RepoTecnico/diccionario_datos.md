...
> **v1.9 (esta versión):** **asienta las ampliaciones de modelo que la implementación de H2 declaró** y **cierra los huecos del catálogo de errores** que H2 reportó. **(1)** §2.10 gana la fila canónica `hiddenAccounts: number[]` —índices BIP-44 de las cuentas derivadas **ocultas** (RF-06)—, junto a `accountLabels`; el reset de la cartera la limpia con el resto de `truekeate_settings` (RF-32). **(2)** §4.3 gana el bloque **«Causas añadidas en la v1.9 (H2)»** con **siete filas nuevas** —«cartera dañada», «no hay cartera», «referencia inexistente», «importe inválido», «etiqueta inválida», «fallo del portapapeles» y «fallo de escritura de migración»—, que reutilizan códigos ya registrados (`-32602`, `-32000` y `-32603`), de modo que el catálogo sigue teniendo **8 códigos** y **25 filas en la tabla del núcleo**. Detalle en §7.
