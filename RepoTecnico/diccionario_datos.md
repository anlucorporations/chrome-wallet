...
> **v1.6:** cierra el bloque 3 del veredicto de reevaluación de la Fase 2: el delta del **evento 24** (`storage_quota_exceeded`) queda espejado en `requerimientos.md` §2.2 y en el documento técnico, **§4.3 queda confirmada como fuente única de los literales** y el seguimiento de `eth_getTransactionReceipt` gana criterio observable. Detalle en §7.
> **v1.7:** cierra el residual **`R-09`** del veredicto con las decisiones **DEC-45** y **DEC-46**: **dos filas nuevas** en la tabla de errores de §4.3 (una por causa: «Cuenta en uso por una dApp conectada» y «Reset bloqueado»), la **guarda de sesión de dApp activa** en el revelado/exportación y en el borrado de una cuenta importada (§3.10), y la **guarda y el orden de comprobación del reset** con lo que se limpia y lo que se conserva (§3.11). Detalle en §6.5 y §7.
> **v1.8:** cierra los residuales de la última pasada de consistencia **`VR-01`** (los permisos `favicon`, `clipboardRead` y `clipboardWrite` quedan **declarados y justificados** en `documento_tecnico.md` §7.3 y `entornos_globales.md` §4, y las constantes que este documento citaba de `entornos_globales.md` §3 —`EXTENSION_ID`, `INFLIGHT_TTL_MS`, `PREVIEW_INLINE_MAX_BYTES` y las cuatro del limitador de tasa— ya existen allí), **`VR-03`** (referencias colgantes cerradas) y **`VR-06`** (la afirmación no falsable «mantiene vivo el SW» se sustituye por la formulación verificable de §3.4). Los cuatro **deltas** que este documento señalaba al resto del corpus quedan **cerrados** (§6.5).
> **v1.9:** **asienta las ampliaciones de modelo que la implementación de H2 declaró** y **cierra los huecos del catálogo de errores** que H2 reportó. **(1)** §2.10 gana la fila canónica `hiddenAccounts: number[]` —índices BIP-44 de las cuentas derivadas **ocultas** (RF-06)—, junto a `accountLabels`; el reset de la cartera la limpia con el resto de `truekeate_settings` (RF-32). **(2)** §4.3 gana el bloque **«Causas añadidas en la v1.9 (H2)»** con **siete filas nuevas** —«cartera dañada», «no hay cartera», «referencia inexistente», «importe inválido», «etiqueta inválida», «fallo del portapapeles» y «fallo de escritura de migración»—, que reutilizan códigos ya registrados (`-32602`, `-32000` y `-32603`), de modo que el catálogo sigue teniendo **8 códigos** y **25 filas en la tabla del núcleo**. Detalle en §7.
> **v1.10 (esta versión):** **cierra los dos huecos del catálogo de errores que la implementación de H4 reportó**, sin tocar las filas ya registradas. §4.3 gana el bloque **«Causas añadidas en la v1.10 (H4)»** (§4.3.2) con **dos filas nuevas** y código compartido `-32000`: **`inflightTxInProgress`** —2.ª aprobación de una cuenta con la marca de `truekeate_inflight_tx` en `phase: 'signing'` vigente (§2.12), que hasta ahora se clasificaba como `tooManyPendingRequests`— y **`broadcastRejected`** —el nodo rechaza una transacción ya firmada en `eth_sendRawTransaction`, con `<motivo>` accionable (§3.6)—, que hasta ahora vivía en una constante local de `rpc/txContract.ts`. El catálogo sigue teniendo **8 códigos** y la tabla del núcleo sus **25 filas** intactas; la transcripción vive en `H4_ERROR_CATALOG` de `src/background/rpc/errors.ts`. Detalle en §7.
...
| `hiddenAccounts` | `number[]` | `[]` | **Nuevo (v1.9, RF-06 / DEC-35).** Índices BIP-44 de las cuentas derivadas **ocultas**: una cuenta derivada **nunca se elimina**, solo se oculta, y su dirección permanece en `truekeate_accounts`. Es la clave canónica de la visibilidad de las derivadas, **junto a `accountLabels`**; las importadas usan su propio campo `visible` (§2.3). El **reset de la cartera la limpia** con el resto del objeto, porque `truekeate_settings` se elimina por completo y solo `truekeate_logs` sobrevive (RF-32). |
...
> **Alineación con el plan de desarrollo (v1.13 · fleco 1 de la Fase 4).** `plan_desarrollo.md` §3.13 es la fuente **autoritativa** y fija **6 solicitudes / 60 s por origen** para el *token bucket* de todo el catálogo; esta sección declaraba además `rateLimitBurst = 20` con recarga lineal `5/s`, constantes que **ningún módulo importaba** y que contradecían aquel límite. Se **eliminan** las dos constantes muertas y la ventana queda con **una sola fuente** (`rateLimitWindowRequests = pendingRequestsPerMinute`, `rateLimitWindowMs = RATE_WINDOW_MS`) en `src/shared/constants.ts`. La defensa **no se debilita**: el límite efectivo es el **más estricto** de los dos que ya regían (6 solicitudes por ventana de 60 s), la exención de `origin === 'extension'` y la purga por inactividad se conservan intactas, y los tests de la ventana (`rateLimit.spec.ts`, `approvalRateWindow.spec.ts`) no se relajan.

| Constante | Valor | Ámbito |
|---|---|---|
| `rateLimitWindowRequests` | `6` | Solicitudes por origen y ventana del *token bucket* de todo el catálogo (1 token = 1 llamada RPC de página). Es el MISMO valor que `pendingRequestsPerMinute` (§2.8): una sola fuente. |
| `rateLimitWindowMs` | `60000` | Ventana del *token bucket* (`RATE_WINDOW_MS`): 6 solicitudes por ventana de 60 s por origen. |
| `rateWindowTtlMs` | `600000` | Purga de entradas inactivas (10 min sin uso = ventana reiniciada). |
| `RATE_PERSIST_DEBOUNCE_MS` | `1000` | *Debounce* máximo de escritura para los métodos **no** aprobables (máx. 1 escritura/s por origen). |

**Algoritmo (ventana deslizante persistida) y decisión.** La implementación (`src/background/rpc/rateLimit.ts`, M3.b) es una **ventana** con reinicio perezoso, no un cubo con recarga proporcional: el corpus fija el número de solicitudes por ventana, no una tasa de recarga, así que no hay recarga parcial que calcular.

```
// `normalizeRateWindow(stored, now)`
si no hay entrada          → ventana nueva: tokens = 6, approvalWindowStart = now, approvalsInWindow = 0
si el reloj está en el futuro (lastRefillAt > now o approvalWindowStart > now) → se reinicia por completo
si now - approvalWindowStart >= 60000 → se reinicia: tokens = 6, approvalWindowStart = now, approvalsInWindow = 0
en otro caso               → tokens = min(tokens, 6)

// `consumeRateWindow(...)`
si tokens <= 0  →  rechazo 4001 (se incrementa deniedCount; NO se ejecuta la llamada)
si tokens >  0  →  tokens = tokens - 1  (la llamada consume 1 token)
```
...
**Escritura y reconstrucción.** Solo el SW escribe, con la RMW serializada de §2.8; los métodos **no** aprobables se persisten con `RATE_PERSIST_DEBOUNCE_MS = 1000` (máximo 1 escritura por segundo y origen) y los aprobables, en el mismo `set` de la cola. Al arrancar el SW: (1) se lee el mapa; (2) se descartan las entradas con `now - updatedAt > rateWindowTtlMs` (equivale a ventana reiniciada); (3) si `lastRefillAt` o `approvalWindowStart` están en el futuro (reloj movido hacia atrás) la entrada se reinicia a `{ tokens: 6, lastRefillAt: now, approvalWindowStart: now, approvalsInWindow: 0 }`; (4) si `now - approvalWindowStart >= 60000` la ventana se reinicia con `approvalsInWindow = 0`. Residuo: una suspensión puede perder hasta 1 s de contabilidad; la ventana nunca es la única defensa, porque la cardinalidad de RF-37/§2.8 sigue aplicando.

*Test de aceptación:* 100 llamadas de lectura seguidas desde el mismo origen → la llamada 7 se rechaza con `4001` y la ventana no se reinicia hasta que transcurren 60 s (medido con reloj inyectado sobre `truekeate_rate_windows`).
...
|---|---|---|---|
| `portsByApprovalId` | `Map<string, chrome.runtime.Port>` | Puerto de larga vida abierto por el content script o el popup que espera la respuesta (`chrome.runtime.connect({ name: 'truekeate_approval' })`). El puerto **no** garantiza la vida del Service Worker (el navegador lo suspende tras ~30 s de inactividad): tras una suspensión, un `RESUME` con el mismo `approvalId` obtiene respuesta en < 200 ms y el vencimiento se rearma con `chrome.alarms`. | Si el SW se duerme, el puerto se cierra: el content script se **reconecta con backoff** (1 s, 2 s, 4 s, 8 s, 16 s, máx. 30 s) y envía `{ type: 'RESUME', approvalId }`; el SW contesta desde el registro persistido o con `4001` si la entrada ya no está `pending`. |
...
**Guarda de sesión de dApp activa (R-09a / DEC-45).** El revelado y la exportación quedan **bloqueados** si la cuenta está **en uso por una dApp**: existe una entrada **vigente** en `truekeate_connected_sites` (§2.7) cuya cuenta autorizada es la cuenta revelada (`kind: 'privateKey'`) o, cuando se revela el **mnemonic**, cualquiera de las cuentas que este deriva. En ese caso el valor **no se entrega** y se responde con el error tipado **`-32000`** y el literal de la causa «Cuenta en uso por una dApp conectada» de **§4.3** —fuente única de los literales—, que nombra el `origen` de la dApp y la acción (revocar su permiso). La comprobación se hace **en el popup al abrir la acción** y se **revalida en el SW** al resolver el secreto (defensa en profundidad). La misma guarda rige el **borrado de una cuenta importada** (RF-06): ninguna de las dos operaciones se ofrece ni se ejecuta mientras la sesión siga vigente; revocar el permiso de esa dApp en «Sitios conectados» es la única salida. Una entrada se considera **vigente** mientras exista y no haya vencido (`expiresAt > now`; 24 h renovables en cada uso, §2.7).

### 3.11 Reset de la cartera: guardas y orden de comprobación (R-09b / DEC-46)

**Requisito:** RF-11 (Must) + `CA-RF-11`; caso de uso CU-30. **Guardas de estado (DEC-46):** el reset **no se inicia** si `truekeate_pending_requests` (§2.8) tiene alguna entrada en estado `pending`, ni si `truekeate_inflight_tx` (§2.12) tiene alguna entrada vigente (TTL de 180 s). Es un **error de validación de UI** —no lo devuelve ningún método RPC— con el error tipado **`-32000`** y el literal de la causa «Reset bloqueado» de **§4.3**; la UI lo muestra con el **número exacto** de solicitudes pendientes y con las dos salidas posibles. **No se permite continuar.**

**Orden de comprobación (estricto, antes de tocar el storage).**

1. **Cola vacía** — `truekeate_pending_requests` sin entradas `pending` (§2.8). Si hay `n > 0`, **bloqueo** con `-32000` y el literal de «Reset bloqueado», cuyo contador `<n>` es ese número.
2. **Sin transacción en vuelo** — `truekeate_inflight_tx` sin entradas vigentes (§2.12; se borran al confirmar o fallar y las purga la reconciliación al arrancar). Si las hay, **bloqueo** con el mismo código y literal.
3. **Confirmación destructiva** — solo con (1) y (2) en verde se abre el diálogo destructivo, que **enumera** las cuentas importadas que se perderán (RNF-22). Cancelar no modifica nada.
...
**Salidas que la UI ofrece durante el bloqueo (DEC-46).** (a) **Resolver ahora**: llevar al usuario a la cola —la ventana única `notification.html`— para **aprobar o rechazar** cada solicitud; (b) **Esperar**: los plazos del SW cierran las solicitudes con `4001` (§2.3, §3.4), la cola queda vacía y el reset procede. Mientras la guarda esté activa **no se elimina ninguna clave**.

**Qué se limpia y qué se conserva.**

| Grupo | Claves | Efecto del reset |
|---|---|---|
| Material de la cartera | `truekeate_mnemonic`, `truekeate_accounts`, `truekeate_imported_accounts`, `truekeate_current_account` (§2.1–§2.4) | **Se eliminan** |
| Red | `truekeate_chain_id`, `truekeate_networks` (§2.5–§2.6) | **Se eliminan** (se vuelve al default Anvil Local) |
| Sesiones y colas | `truekeate_connected_sites`, `truekeate_pending_requests`, `truekeate_connect_request`, `truekeate_approval_window` (§2.7–§2.9, §2.14) | **Se eliminan** (la cola ya está vacía por la guarda del paso 1) |
| Estado operativo | `truekeate_settings`, `truekeate_inflight_tx`, `truekeate_rate_windows` (§2.10, §2.12–§2.13) | **Se eliminan** (`truekeate_inflight_tx` ya está vacío por la guarda del paso 2) |
| Auditoría | `truekeate_logs` (§2.11) | **Se conserva** (**RF-32**, ADR-11): el histórico sobrevive al reset y se exporta en JSON |

> **Idempotencia y fallo parcial.** El reset es **idempotente**: repetirlo con el storage ya limpio es un no-op que vuelve a registrar `reset_wallet`. Si una escritura falla, el SW reintenta **una vez** y, si persiste, responde `-32603` con el literal de la causa «Fallo no clasificado del SW» de §4.3, informa en la UI y deja la traza del estado parcial en `truekeate_logs`.
...
| `-32000` | **Cuenta en uso por una dApp conectada**: entrada vigente en `truekeate_connected_sites` para esa cuenta, que **bloquea** el revelado/exportación de su clave privada (o del mnemonic que la deriva) y la **eliminación** de la cuenta importada (R-09a / DEC-45, §3.10) | «La cuenta está en uso por la dApp `<origen>`: revoca ese permiso antes de revelar su clave privada o eliminarla.» | Revocar la sesión de esa dApp en «Sitios conectados» y reintentar |
| `-32000` | **Reset bloqueado**: `truekeate_pending_requests` con `<n>` solicitudes `pending`, o una transacción en vuelo vigente en `truekeate_inflight_tx`; es un **error de validación de UI**, no lo devuelve ningún método RPC (R-09b / DEC-46, §3.11) | «No se puede resetear la cartera: quedan `<n>` solicitudes pendientes o una transacción en vuelo. Resuélvelas (aprueba o rechaza) o espera a que expiren.» | Resolver la cola (aprobar o rechazar cada solicitud) o esperar al vencimiento del plazo y reintentar |
...
#### 4.3.1 Causas añadidas en la v1.9 (H2) — segunda tabla de la misma fuente única

La implementación de H2 (cartera, cuentas y recuperación) reportó **siete causas sin fila** en la tabla del núcleo, que hasta ahora obligaban a degradar el fallo a «Error interno de la cartera» o a inventar un literal en la UI. Se registran aquí, **dentro de §4.3** y con el mismo formato, porque esta sección sigue siendo la **fuente única** de los `message` en español. **Ninguna añade código nuevo**: reutilizan los ocho códigos del catálogo (un `code` admite varios mensajes, uno por causa).

| Código | Causa | Mensaje (español) | Acción sugerida |
|---|---|---|---|
| `-32603` | **Cartera dañada** (`damagedWallet`): el almacén persistido no supera la comprobación de integridad al arrancar —checksum BIP-39 del mnemonic o checksum EIP-55 de una dirección— (RNF-22, §2.1/§2.2) | «La cartera guardada está dañada y no se puede usar: no se derivan cuentas nuevas desde ella.» | Restaurar la cartera desde la frase de recuperación o resetearla |
| `-32000` | **No hay cartera** (`walletNotCreated`): se pide derivar una cuenta y no existe ni frase semilla ni cuentas (RF-04) | «Todavía no hay ninguna cartera: no se puede derivar ninguna cuenta.» | Crear una cartera nueva o importar una frase de recuperación |
| `-32602` | **Referencia inexistente** (`unknownAccount`): la referencia de cuenta (`idx:<n>` / `imp:<address>`) no corresponde a ninguna cuenta de la cartera (RF-06, §2.4); lo devuelven `wallet_setCurrentAccount`, `wallet_renameAccount`, `wallet_setAccountVisible`, `wallet_deleteImportedAccount` y el revelado de RF-50 | «La cuenta indicada no existe en la cartera.» | Elegir una cuenta de la lista |
| `-32602` | **Importe inválido** (`invalidAmount`): el importe no es un decimal positivo con hasta 18 decimales (RF-33; validación inline de M60) | «El importe no es válido: usa un número decimal positivo con hasta 18 decimales.» | Revisar el importe |
| `-32602` | **Etiqueta inválida** (`invalidLabel`): la etiqueta de una cuenta queda fuera de 1..32 caracteres o contiene caracteres de control (RF-06 / ACU-06, DEC-35) | «La etiqueta no es válida: debe tener entre 1 y 32 caracteres.» | Acortar o corregir la etiqueta |
| `-32603` | **Fallo del portapapeles** (`clipboardFailure`): no se pudo copiar el valor revelado ni limpiar el portapapeles al ocultarlo (P-20 / §3.10) | «No se pudo usar el portapapeles: el valor no se ha copiado o no se ha podido borrar.» | Comprobar el permiso del portapapeles y reintentar; borrar el valor a mano si estaba copiado |
| `-32603` | **Fallo de escritura de la migración** (`migrationWriteFailed`): la migración de esquema se planificó pero no se pudo escribir en `chrome.storage.local` (M34, `documento_tecnico.md` §4.3) | «No se pudo guardar la migración del esquema: los datos se han quedado sin actualizar.» | Exportar los logs en JSON y reintentar; si persiste, resetear la cartera |

> **Nota de estructura (trazabilidad).** Las **25 filas del núcleo** de §4.3 se mantienen **íntegras y en su orden**: son las que transcribe, fila a fila, el catálogo de M6 (`src/background/rpc/errors.ts`, `ERROR_CATALOG`) y las que verifica `errors.spec.ts`. Estas siete causas se transcriben aparte en `EXTENDED_ERROR_CATALOG`, y el resolutor de M6 (`createEip1193Error`) busca en **ambas**, de modo que la fuente única sigue siendo §4.3 y ningún literal vive solo en el código. La UI las repite en `src/popup/popupErrors.ts` (el popup no puede importar del SW: RNF-14) y busca la acción sugerida **primero por mensaje** y después por código, porque varias causas comparten `-32602` y `-32603`.

#### 4.3.2 Causas añadidas en la v1.10 (H4) — tercera tabla de la misma fuente única

La implementación de H4 (cola de aprobación, transacción en vuelo y contrato observable de la transacción) reportó **dos causas sin fila** en la tabla del núcleo. Se registran aquí, **dentro de §4.3** y con el mismo formato, porque esta sección sigue siendo la **fuente única** de los `message` en español. **Ninguna añade código nuevo**: ambas reutilizan `-32000` (conflicto de estado), de modo que el catálogo sigue teniendo **8 códigos** y **25 filas en la tabla del núcleo**.

| Código | Causa | Mensaje (español) | Acción sugerida |
|---|---|---|---|
| `-32000` | **Transacción en vuelo de la misma cuenta** (`inflightTxInProgress`): una 2.ª solicitud de firma de una cuenta cuya marca en `truekeate_inflight_tx` sigue en `phase: 'signing'` y vigente, es decir, antes de que la primera se difunda (§2.12 reglas 2 y 5). Hasta la v1.10 se clasificaba como `tooManyPendingRequests`, que describe un exceso de cardinalidad y no este conflicto | «Ya hay una transacción de esta cuenta en vuelo; espera a que se difunda antes de firmar otra.» | Esperar a que la transacción en vuelo se difunda y reintentar |
| `-32000` | **Difusión rechazada por el nodo** (`broadcastRejected`): `eth_sendRawTransaction` devuelve un error de la red sobre una transacción **ya firmada** —`already known`, `replacement transaction underpriced`, `intrinsic gas too low`, `max fee per gas less than block base fee`…—, causa distinta del nonce inválido y de la difusión interrumpida (§3.6, H4/M7). El `<motivo>` es el texto accionable del nodo | «La red rechazó la transacción: <motivo>.» | Revisar el motivo indicado y volver a intentarlo |

> **Nota de estructura (trazabilidad).** Las **25 filas del núcleo** y las **7 de la v1.9** siguen **íntegras y en su orden**. Estas **dos causas de la v1.10** se transcriben aparte en `H4_ERROR_CATALOG` (`src/background/rpc/errors.ts`) y el resolutor de M6 busca en **las tres** tablas, de modo que la fuente única sigue siendo §4.3 y ningún literal vive solo en el código: `rpc/txContract.ts` **ya no** mantiene una constante local de `broadcastRejected` (usa `broadcastRejectedError`/`errorDefinitionFor` de M6) y `approvals/queue.ts` responde `inflightTxInProgress` en lugar de `tooManyPendingRequests` cuando la cuenta ya está firmando. La UI repite ambas en `src/popup/popupErrors.ts`.

#### 4.3.3 Causas añadidas en la v1.11 (H5) — cuarta tabla de la misma fuente única

La implementación de H5 (redes) reportó **tres causas sin fila**. Se registran aquí, **dentro de §4.3** y con el mismo formato, porque esta sección sigue siendo la **fuente única** de los `message` en español; **ninguna añade código nuevo** (reutilizan `-32602` y `4901`, así que el catálogo conserva **8 códigos**).

| Código | Causa | Mensaje (español) | Acción sugerida |
|---|---|---|---|
| `-32602` | **`rpcUrl` inválido** (`invalidRpcUrl`): la dirección del nodo no supera la validación previa de §3.5 (esquema que no es `http`/`https`, `http` en claro fuera de `127.0.0.1`/`localhost`, host privado RFC 1918 o de enlace local RFC 3927, o URL malformada). Se rechaza **sin** abrir ventana y **sin** pedir permiso de host | «La dirección del nodo no es válida: usa `https` o, solo para el RPC local, `http` en `127.0.0.1`/`localhost`.» | Revisar el `rpcUrl` de la red y volver a darla de alta |
| `-32602` | **Definición de red inválida** (`invalidNetworkDefinition`): `chainId` ausente, malformado o incoherente entre su forma hexadecimal y su decimal, nombre o símbolo no imprimibles, o `params[0]` que no es un objeto con nombre (EIP-3085) | «Los datos de la red no son válidos: revisa el `chainId`, el nombre y el símbolo de la moneda nativa.» | Revisar los datos declarados por la dApp |
| `4901` | **Incoherencia de red** (`chainIdMismatch`): el nodo del `rpcUrl` declara un `chainId` **distinto** del que declara la red, así que la red **no queda utilizable** y NO se persiste (coherencia de §3.5) | «La red solicitada no está dada de alta.» — la fila REUTILIZA el literal de `chainNotRegistered`, que es exactamente lo que ocurre desde el punto de vista del usuario; el `data` del error conserva `{ reason: 'chain-id-coherence', declared, observed }` para diagnóstico | Darla de alta con `wallet_addEthereumChain` |

> **Nota de estructura (trazabilidad).** La implementación de las tres (`invalidRpcUrlError`, `invalidNetworkDefinitionError` y `chainIdMismatchError`) vive en `H5_ERROR_CATALOG` de `src/background/rpc/errors.ts`. **Desviación documental detectada y declarada en la Fase 4**: el código de H5 se escribió citando esta subsección antes de que existiera, y `chainIdMismatch` usa el `message` de `chainNotRegistered` en vez de un literal propio; se deja **constancia expresa** aquí en vez de inventar un mensaje nuevo (regla ACU-05: el literal no se inventa fuera de esta tabla). Es una **divergencia de declaración**, no de comportamiento: el requisito «el nodo declara otro `chainId` → la red no es utilizable» se cumple y está cubierto por `networks.spec.ts`.

#### 4.3.4 Causas añadidas en la v1.13 (Fase 4) — quinta tabla de la misma fuente única

La fase de pruebas (Fase 4) midió en E2E (`e2e/33-envio-desde-el-popup.spec.ts`, evidencia en `RepoTecnico/evidencia/Fase4/33-envio-desde-el-popup-2026-09-13.json`) una causa **sin fila**: cuando el nodo **rechaza** una llamada con un error JSON-RPC determinista (`-32003 Insufficient funds for gas * price + value`, y en general cualquier código del rango reservado `-32768…-32000`), el cliente RPC gastaba la política completa de reintentos, la degradaba a `4900` y el usuario veía «Sin conexión con la red local (Anvil).» con la UI en «desconectado» —cuando **el nodo sí había respondido**—. Se registra aquí, **dentro de §4.3**, porque esta sección sigue siendo la **fuente única** de los `message` en español. **No añade ningún código nuevo**: reutiliza `-32000` (conflicto de estado / fallo de la llamada).

| Código | Causa | Mensaje (español) | Acción sugerida |
|---|---|---|---|
| `-32000` | **Rechazo del nodo en una lectura** (`nodeRejected`): el nodo **respondió** con un error JSON-RPC determinista a una llamada de página —`eth_estimateGas` con `-32003 Insufficient funds for gas * price + value`, un revert, `-32601`, …—, es decir, la llamada llegó y fue rechazada. NO es «sin conexión»: la conexión con el nodo es correcta y **no se reintenta**, porque un error de respuesta no cambia por reintentarlo (antes gastaba 4 intentos y 7 s de backoff, `e2e/33`) | «La estimación de gas falló: <motivo>. El envío se ha bloqueado.» — el `<motivo>` es el texto accionable del nodo; se REUTILIZA el literal ya registrado de `estimateGasFailed` (tabla del núcleo), que ya cubría «`estimateGas` fallido o revert previo a firmar» | Corregir la llamada |

> **Nota de estructura (trazabilidad).** Las **25 filas del núcleo**, las **7 de la v1.9**, las **2 de la v1.10** y las **3 de la v1.11** siguen **íntegras y en su orden**; el catálogo sigue teniendo **8 códigos** y la tabla del núcleo **25 filas**. Esta fila **no crea ninguna causa nueva en `errors.ts`**: el rechazo del nodo se detecta con `nodeRejectionOf`/`findNodeRejection` (`src/background/rpc/client.ts`), que reconoce el rango reservado del servidor JSON-RPC y lo distingue de los códigos PROPIOS de la cartera (`4001`, `4100`, `4200`, `4900`, `4901`), y se entrega con el literal ya registrado (`estimateGasFailed` en `pageMethods.ts`; `broadcastRejected` donde corresponde). Regla dura respetada: **ningún literal nuevo fuera de esta tabla**.
...
### 6.5 Cierre de `R-09` (decisiones DEC-45 y DEC-46)

| Causa (disparador) | Código | Fila en §4.3 | Guarda que la emite |
|---|---|---|---|
| **Cuenta en uso por una dApp conectada** (entrada vigente en `truekeate_connected_sites` para la cuenta) | `-32000` | «La cuenta está en uso por la dApp `<origen>`: revoca ese permiso antes de revelar su clave privada o eliminarla.» | **Bloquea** el revelado/exportación (RF-50) y el borrado de la cuenta importada (RF-06) — §3.10 / DEC-45 |
| **Reset bloqueado** (cola `truekeate_pending_requests` no vacía, o transacción en vuelo en `truekeate_inflight_tx`) | `-32000` | «No se puede resetear la cartera: quedan `<n>` solicitudes pendientes o una transacción en vuelo. Resuélvelas (aprueba o rechaza) o espera a que expiren.» | **Bloquea** el reset (RF-11) — §3.11 / DEC-46; es **error de validación de UI** y ningún método RPC lo devuelve |

**Justificación del código elegido (`-32000`).** En este diccionario `-32000` es la familia de **conflicto de estado**: se emite cuando la entrada es válida pero el *estado actual* del sistema impide la operación (saldo insuficiente, nonce inválido, `estimateGas` fallido). Las dos causas nuevas son exactamente de esa naturaleza —la cuenta es revelable y eliminable en abstracto, y el reset es legítimo, pero una sesión de dApp vigente o una cola con trabajo en vuelo lo impiden **ahora**—, así que se alinean con las filas ya existentes. Se descartan `-32602` (reservado a **material de entrada inválido**: mnemonic, clave privada, dirección, payload) y `-32603` (reservado a **fallos internos** del SW o de la cuota). La causa del reset **no la devuelve ningún método RPC** —es una precondición de UI—, pero por la regla de §4.3 («todo error que se muestre al usuario lleva `code` numérico») viaja igualmente como error tipado con `code: -32000` y con su literal en esta misma tabla: **no se inventa ningún código de método**.

**Deltas al resto del corpus: cerrados y verificados en la v1.8 (`VR-01`/`VR-03`).** Los **cuatro** deltas que este documento señalaba **ya no están abiertos**: (1) el **evento 24** (`storage_quota_exceeded`) y la fila `-32602` de payload están espejados en `requerimientos.md` §2.2 y en el catálogo de `documento_tecnico.md`; (2) los permisos **`favicon`**, **`clipboardRead`** y **`clipboardWrite`** están declarados y justificados en `documento_tecnico.md` §7.3 (tabla de permisos y fragmentos de manifest), en `entornos_globales.md` §4 y en `requerimientos.md` RT-04/`CA-RT-04`, de modo que el conjunto de permisos es **idéntico en los cuatro documentos** —`["storage", "alarms", "favicon", "clipboardRead", "clipboardWrite"]`, con `notifications` **solo** en `optional_permissions`—; además, las constantes que este documento cita de `entornos_globales.md` §3 (`MAX_PAYLOAD_BYTES`, `PREVIEW_INLINE_MAX_BYTES`, `REVEAL_HIDE_MS`, `INFLIGHT_TTL_MS`, `rateWindowTtlMs`, `RATE_PERSIST_DEBOUNCE_MS`, `PROVIDER_UUID` y `EXTENSION_ID`) están **todas** declaradas allí, así que **ninguna referencia queda colgante**; las dos constantes del *token bucket* que se citaban en esa lista (`rateLimitBurst` y `rateLimitRefillPerSecond`) fueron **eliminadas en la v1.13** al alinearse §2.13 con la fuente autoritativa (6/60 s), de modo que tampoco quedan referencias a constantes muertas; (3) RF-21 y RF-50 recogen la redacción de previews largas y la política de portapapeles; y **(4)** las guardas de `R-09` (`DEC-45`/`DEC-46`) se citan desde `requerimientos.md` (`CA-RF-05`, `CA-RF-06`, `CA-RF-11` y `CA-RF-50`), `casos_uso.md` (CU-06, CU-07 y CU-30) y `documento_tecnico.md` (guarda del revelado en §3.8 y flujo de reset en §3.9), siempre por la **causa** o por el **código** y **sin reproducir el literal**, cuya fuente única es §4.3. **No queda ningún delta abierto.**
 ...
 **1.6** — cierre de los residuales del veredicto de reevaluación: el delta del **evento 24** (`storage_quota_exceeded`) se declara **ya espejado** en `requerimientos.md` §2.2 y en el `documento_tecnico.md` (M31); **§4.3 queda confirmada como fuente única de los literales** de error; el seguimiento de `eth_getTransactionReceipt` de §3.6 gana criterio observable (recibo `0x1`/`0x0` o vencimiento del plazo persistido, 1 entrada de log por transición); y se traza el conteo histórico 23 → 24 en el historial.
 **1.8** — cierre de los residuales **`VR-01`**, **`VR-03`** y **`VR-06`** de la última pasada de consistencia: el conjunto de permisos queda **unificado** con `documento_tecnico.md` §7.3 y `entornos_globales.md` §4 (`favicon`, `clipboardRead` y `clipboardWrite`, además de `storage` y `alarms`), las constantes del limitador de tasa, del payload y del ID de la extensión quedan **declaradas** en `entornos_globales.md` §3 (ninguna cita colgante), la afirmación no falsable del puerto se sustituye por la formulación verificable («tras ~30 s de inactividad, un `RESUME` con el mismo `approvalId` responde en < 200 ms y el vencimiento se rearma con `chrome.alarms`», §3.4) y los **cuatro deltas** al resto del corpus quedan **cerrados** (§6.5). con las decisiones **DEC-45** y **DEC-46**: **dos filas nuevas** en la tabla de errores de **§4.3** (una por causa) —«Cuenta en uso por una dApp conectada» y «Reset bloqueado», ambas con `code: -32000` por ser conflictos de estado—; **guarda de sesión de dApp activa** en el revelado/exportación y en el borrado de la cuenta importada (**§3.10**); y **§3.11 nueva**: guardas del reset, orden estricto de comprobación (cola vacía → sin transacción en vuelo → confirmación destructiva → limpieza), salidas ofrecidas al usuario, y tabla de lo que se limpia y lo que se conserva (`truekeate_logs` sobrevive por **RF-32**). Detalle y justificación del código en **§6.5**.

 **1.9 (esta versión)** — **asentamiento del modelo de H2** y **cierre de los huecos del catálogo de errores**: **(1)** nueva fila canónica **`hiddenAccounts: number[]`** en `truekeate_settings` (**§2.10**), junto a `accountLabels`, para la visibilidad de las cuentas derivadas (RF-06 / DEC-35), con la nota de que el **reset la limpia** con el resto del objeto (RF-32); **(2)** nueva subsección **§4.3.1** con **siete causas** que H2 reportó como ausentes —«cartera dañada» (`damagedWallet`, `-32603`), «no hay cartera» (`walletNotCreated`, `-32000`), «referencia inexistente» (`unknownAccount`, `-32602`), «importe inválido» (`invalidAmount`, `-32602`), «etiqueta inválida» (`invalidLabel`, `-32602`), «fallo del portapapeles» (`clipboardFailure`, `-32603`) y «fallo de escritura de migración» (`migrationWriteFailed`, `-32603`)—, **sin añadir ningún código nuevo** (el catálogo sigue con **8 códigos**) y **sin tocar las 25 filas del núcleo**, que son las que transcribe y verifica `errors.spec.ts`. La implementación correspondiente vive en `src/background/rpc/errors.ts` (`EXTENDED_ERROR_CATALOG` y sus ayudantes) y en `src/popup/popupErrors.ts`; el contrato de los métodos internos que las produce se documenta en `documento_tecnico.md` **§5.1.1 v1.6**.

 **1.10 (esta versión)** — **cierre de los dos huecos del catálogo de errores que H4 reportó**: nueva subsección **§4.3.2** («Causas añadidas en la v1.10 (H4)») con **dos filas** y código compartido **`-32000`** (conflicto de estado) —«transacción en vuelo de la misma cuenta» (`inflightTxInProgress`: la 2.ª aprobación de una cuenta con la marca de `truekeate_inflight_tx` en `phase: 'signing'` vigente, §2.12, que hasta ahora se clasificaba como `tooManyPendingRequests`) y «difusión rechazada por el nodo» (`broadcastRejected`: el nodo rechaza una transacción ya firmada en `eth_sendRawTransaction` con `<motivo>` accionable, §3.6, que hasta ahora vivía en una constante local de `rpc/txContract.ts`)—, **sin añadir ningún código nuevo** (el catálogo sigue con **8 códigos**) y **sin tocar las 25 filas del núcleo ni las 7 de la v1.9**. La transcripción vive en `H4_ERROR_CATALOG` de `src/background/rpc/errors.ts` (con los ayudantes `inflightTxInProgressError` y `broadcastRejectedError`) y en `src/popup/popupErrors.ts`; `rpc/txContract.ts` y `approvals/queue.ts` la consumen desde M6. Detalle en §4.3.2.
