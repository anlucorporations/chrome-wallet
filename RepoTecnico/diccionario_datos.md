...
> **Delta de v1.5 que debe reflejarse en el resto del corpus (ADT-14 / D-M).** `storage_quota_exceeded` se instrumenta con `category: 'system'`, `level: 'error'` y `data: { code: -32603, key, bytesInUse, retried: true }` (§2.15). El conteo pasa de **23** a **24** eventos, por lo que `requerimientos.md` §2.2 y el catálogo de eventos del `documento_tecnico.md` (M31) deben listarlo también; hasta que se espeje, este documento es la fuente de verdad del enum.
...
| Seguimiento | El SW consulta `eth_getTransactionReceipt` (método añadido al catálogo, §4.3) hasta obtener recibo o agotar el seguimiento; el resultado se refleja en los logs (§2.11), que es la evidencia verificable de RF-31. Cada transición consulta además la marca persistida `truekeate_inflight_tx` (§2.12) y la limpia al cerrar el ciclo. |
