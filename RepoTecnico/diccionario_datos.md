...
| `expiresAt` | `number \| null` | Epoch ms. Caducidad de la sesión: `lastUsedAt + settings.sessionTtlMs`. `null` = sin caducidad. Al vencer, la sesión se elimina y `eth_accounts` devuelve `[]` (RF-17). |
...
|---|---|---|
| Máximo global de solicitudes `pending` | `settings.pendingRequestsMax = 8` | La nueva solicitud se rechaza de inmediato con `4001` (mensaje en español); **no** se persiste, **no** se abre ventana y **no** cuenta para el badge. Se registra en `truekeate_logs`. |
| Máximo por origen | `settings.pendingRequestsMaxPerOrigin = 1` | La segunda solicitud del mismo origen mientras la primera esté `pending` se rechaza con `4001`. Evita la fatiga de aprobación y la ventana duplicada del mismo origen (H-18, H-35). |
| Límite de tasa por origen | `settings.pendingRequestsPerMinute = 6` | Pasado el umbral en la ventana de 60 s, se rechaza con `4001` sin abrir ventana. |
...
| `sessionTtlMs` | `number` | `86400000` | Caducidad de la sesión de dApp desde `lastUsedAt` (§2.7, H-33). |
...
3. emite a la página un **objeto EIP-1193**, nunca `new Error('Request timeout')`:
   ```json
   { "code": 4001, "message": "El usuario no respondió en el plazo establecido; la solicitud ha caducado." }
   ```
...
| `wallet_switchEthereumChain` | red | Sí (si la red no es la activa) | página | RF-22 |
| `wallet_addEthereumChain` | red | **Sí** | página | RF-23 |
...
|---|---|---|
| `4001` | User rejected request | Rechazo, timeout, cierre de ventana, exceso de cardinalidad o de tasa, caducidad de sesión (RF-40, H-18). El objeto incluye `message` en español (RNF-06). |
| `4100` | Unauthorized | Método que requiere conexión previa sin sesión, o emisor no autorizado (§4.2). |
| `4200` | Unsupported method | Método no implementado (`eth_sign`), método interno invocado desde un contexto no permitido. |
...
| `-32603` | Internal error | Error inesperado del SW, identificador duplicado en la cola (§2.8). |
| `-32000` | Invalid input / funds | Saldo insuficiente, nonce inválido, **fallo de `estimateGas`** o revert detectado antes de firmar (§3.6, H-22). |

*Mensajes de vencimiento (H-07):* el error que llega a la página es siempre un objeto EIP-1193, nunca un `Error` sin `code`:
```json
{ "code": 4001, "message": "El usuario no respondió en el plazo establecido; la solicitud ha caducado." }
```
