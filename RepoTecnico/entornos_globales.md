...
> **Fase:** 1 — Concepto · **Versión:** 2.2
> Registro de configuración, rutas, variables de entorno y comandos importantes. Se actualiza a lo largo del proyecto.
> **v2.2 (esta versión):** **corrige el comando de Anvil del arnés** a la bandera que **sí** existe en **Anvil 1.7.2-dev**: `anvil --host 127.0.0.1 --port 8545 --chain-id 31337 --allow-origin "*"`, **sin `--silent`** (§2.1, 2026-09-12). `--http.corsdomain` **no existe** en esa versión (`error: unexpected argument '--http.corsdomain' found`, medido) y `--allow-origin` admite una **lista separada por comas** y **no puede repetirse**. Detalle de ambos errores en §2.1 y §7.
> **v2.1:** registra el comando de Anvil que se creía válido con el arnés E2E (**H3**, 2026-09-11) —`anvil --host 127.0.0.1 --port 8545 --chain-id 31337 --http.corsdomain "*"`, **sin `--silent`**— con el aviso de que **`anvil --silent` no arranca** cuando se lanza mediante `Start-Process` con redirección de salida (el proceso muere con **exit 1** y toda la suite E2E falla con `ERR_CONNECTION_REFUSED` / `error sending request`), y con la recomendación de que el `global-setup` **aborte con un mensaje claro si Anvil no responde** (§2.1/§2.4). Detalle en §7. **La bandera `--http.corsdomain` de esta versión queda CORREGIDA por la v2.2.**
...
# `--allow-origin` admite UNA lista separada por comas y NO puede repetirse (medido en 1.7.2-dev:
# «the argument '--allow-origin <ALLOW_ORIGIN>' cannot be used multiple times»).
anvil --host 127.0.0.1 --port 8545 --chain-id 31337 `
      --allow-origin "chrome-extension://<ID>,http://localhost:5174,http://127.0.0.1:5174"

# Comando VERIFICADO del arnés E2E (H4, 2026-09-12, Anvil 1.7.2-dev): es el ÚNICO que arranca cuando lo
# lanza la suite. `--allow-origin` es la bandera de CORS de esta versión; `--http.corsdomain` NO existe.
anvil --host 127.0.0.1 --port 8545 --chain-id 31337 --allow-origin "*"

# ❌ ERROR 1 (medido en H4 con Anvil 1.7.2-dev): `--http.corsdomain` NO existe en esta versión. El
#    proceso muere al instante con «error: unexpected argument '--http.corsdomain' found» y la suite
#    entera falla después con ERR_CONNECTION_REFUSED / «error sending request». La bandera
#    equivalente es `--allow-origin` (el comodín se admite SOLO en la máquina del arnés).
# ❌ ERROR 2 (medido en H3): `anvil --silent` NO aparece en `anvil --help` y, lanzado por el arnés con
#    redirección de salida, el proceso puede morir con exit 1. El comando del arnés NO lleva `--silent`.
...
> ⚠️ **No exponer el RPC local (H-41).** `--allow-origin "*"` permitiría a **cualquier** sitio visitado en el equipo llamar al JSON-RPC local (y facilita escenarios de *DNS rebinding* hacia `127.0.0.1`). Se usa siempre la **allowlist concreta** de arriba. El comodín solo sería aceptable en una máquina de desarrollo aislada y sin navegación a sitios de terceros, y en ningún caso se publica el puerto 8545 fuera de `127.0.0.1` (nada de `--host 0.0.0.0`).

> ⚠️ **Comando del arnés, `--http.corsdomain` y `--silent` (H3/H4, DEC-61; corregido en la v2.2).** El comando con **allowlist** de arriba (`chrome-extension://<ID>,http://localhost:5174,http://127.0.0.1:5174`) es la configuración **normativa del producto** (RE-04/H-41) y no cambia. Para la **suite E2E** el comando verificado con **Anvil 1.7.2-dev** (H4, 2026-09-12) es `anvil --host 127.0.0.1 --port 8545 --chain-id 31337 --allow-origin "*"`: ese comodín queda **acotado a la máquina de desarrollo del arnés** (aislada, sin navegación a terceros) y **nunca** se documenta ni se usa como configuración del producto. **Las dos banderas erróneas, medidas:** (1) **`--http.corsdomain` NO existe** en 1.7.2-dev —el proceso muere con `error: unexpected argument '--http.corsdomain' found` y la suite entera falla después con `ERR_CONNECTION_REFUSED` / `error sending request`, un síntoma que se confunde con un defecto de la extensión—; la bandera equivalente es **`--allow-origin`**, que admite una **lista separada por comas** y **no puede repetirse**; y (2) **`--silent`** no aparece en `anvil --help` y, con redirección de salida, el proceso puede morir con **exit 1**, así que el comando del arnés **no** lo lleva.
 ...
 **2.1** — **comando de Anvil del arnés E2E (H3, 2026-09-11)**: `anvil --host 127.0.0.1 --port 8545 --chain-id 31337 --http.corsdomain "*"` **sin `--silent`** (§2.1), con el aviso de que **`anvil --silent` no arranca** cuando se lanza mediante `Start-Process` con redirección de salida (exit 1 → `ERR_CONNECTION_REFUSED` / `error sending request` en toda la suite) y con la **recomendación vinculante** de que el `global-setup` **aborte con un mensaje claro si Anvil no responde** en lugar de solo avisar (§2.4). Decisión **DEC-61** de `estado_proyecto.md` v2.2; sin cambios en permisos, constantes ni política de versiones.
 **2.2 (esta versión)** — **corrección del comando de Anvil (H4, 2026-09-12)**: `--http.corsdomain` **no existe en Anvil 1.7.2-dev** —medido: el proceso muere con `error: unexpected argument '--http.corsdomain' found` y la suite entera falla después con `ERR_CONNECTION_REFUSED` / `error sending request`—, así que el comando del arnés pasa a **`anvil --host 127.0.0.1 --port 8545 --chain-id 31337 --allow-origin "*"`**, **sin `--silent`** (§2.1), y la allowlist normativa usa la MISMA bandera con lista separada por comas (no repetible). Se documentan **los dos errores** (bandera inexistente y `--silent`), se registra que `--http.corsdomain` **no** es válida y se mantiene intacta la advertencia de no exponer el RPC local (H-41/RE-04). Sin cambios en permisos, constantes ni política de versiones.
