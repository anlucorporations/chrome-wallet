...
> **Fase:** 3 — Desarrollo (especificación de Fase 2, corregida al implementar H1, ampliada al implementar H2 y ajustada al implementar H3) · **Versión:** 1.7 · **Estado:** ✅ auditoría `AUDITORIA_DOCUMENTO_TECNICO_V1.md` (ADT-01..ADT-33) aplicada, **correcciones de H1**, **ampliación del contrato interno de H2** y **cierre de los dos huecos de contrato de H3** incorporados — ver §10.4 «Historial de cambios»
> **Cambios de la v1.7 (resumen).** **Cierre de los dos huecos de contrato que bloqueaban H3** (2026-09-11): **(1)** `wallet_getState` publica **`connectedSites`** —las sesiones de `truekeate_connected_sites` en la forma canónica de `diccionario_datos.md` §2.7, con el **origen normalizado**, `current` = sesión vigente y **sin secretos**—, de modo que «Sitios conectados» (M44) pinta la lista real y no un hueco; **(2)** nuevo método interno **`wallet_getConnectRequest`** (`{ requestId }` → `ConnectRequestView`): el Service Worker entrega a `connect.html` la solicitud **completa** de `truekeate_connect_request` (§2.9) por el canal interno que ya existe, y la ventana responde con `account` **y** `accountIndex` calculados sobre esa misma lista (el SW conserva su respaldo por `indexOf(account)`); **(3)** §5.1.1 pasa de **15** a **16** métodos internos y documenta la forma final de `wallet_getState`, la entrega de la solicitud de conexión y sus errores (`4001` con las causas ya registradas en `diccionario_datos.md` §4.3, que sigue siendo la fuente única de los literales). **Sin cambios** en las claves de `chrome.storage.local`, en el catálogo de errores ni en el resto de secciones.
...
> **Regla de frontera (ADT-16).** Toda peticion que cruza el limite popup ↔ SW usa una **union cerrada de metodos**: `ApprovalMethod` (los 6 aprobables) o `InternalMethod` (los **16** internos, seccion 5.1.1: 7 del contrato original, 8 de la ampliacion v1.6 y `wallet_getConnectRequest` en la v1.7). Una cadena suelta `method: string` **no** es contrato valido en esa frontera: `TruekeateRpcMessage.method` se tipa como `ApprovalMethod | InternalMethod | PageMethod`, de modo que el «catalogo cerrado» es verificable **por tipos**, no solo por revision.
...
export type InternalMethod =
  ...
  // Ampliacion de la v1.7: entrega de la solicitud de conexion a connect.html (§2.9)
  | 'wallet_getConnectRequest';     // RF-16/RF-36 (entrega), RNF-14
...
// v1.7: sesion de dApp tal y como la publica el estado: forma canonica de §2.7, SIN secretos.
export interface ConnectedSiteView {
  origin: string;        // normalizado: minusculas, sin barra final (clave canonica de §2.7)
  account: Address;      // cuenta compartida con ese origen
  chainId: ChainIdHex;   // red en el momento de la conexion
  connectedAt: number;   // epoch ms del alta de la sesion
  lastUsedAt: number;    // se refresca en cada uso atendido; base del vencimiento
  expiresAt: number | null;  // lastUsedAt + sessionTtlMs (24 h renovables); null = sin caducidad
  current: boolean;      // true = VIGENTE en la lectura; false = vencida o desconectada
}
// v1.7: solicitud de conexion que el SW entrega a connect.html (subconjunto de §2.9).
export interface ConnectRequestView {
  requestId: Uuid; origin: string; accounts: Address[]; currentAccountIndex: number;
  chainId: ChainIdHex; expiresAt: number;
}
export interface InternalWalletResultMap {
  ...
  wallet_getState: {
    ...
    connectedSites: ConnectedSiteView[];   // v1.7: truekeate_connected_sites (§2.7), SIN secretos
  };
  ...
  wallet_getConnectRequest: ConnectRequestView;   // v1.7; 4001 si no hay solicitud pendiente
}
...
|---|---|---|---|
| `wallet_getState` | — | Estado completo: `hasWallet`, `integrity` (M13), `accounts` (incluidas las ocultas, con `visible` y `current`), `currentAccountRef`, `networks`, `currentChainId`, `settings` y **`connectedSites`** (v1.7: `ConnectedSiteView[]`, la forma canonica de §2.7 **sin secretos**) | `-32603` `damagedWallet`: la cartera dañada se informa en `integrity` y no bloquea la lectura del estado |
...
| `wallet_getConnectRequest` (**v1.7**) | `{ requestId }` | `ConnectRequestView`: `{ requestId, origin (normalizado), accounts, currentAccountIndex, chainId, expiresAt }` de la solicitud `pending` de `truekeate_connect_request` (§2.9). **Solo lectura**: no abre ventana, no crea `PendingRequest` y no escribe en el almacen | `4001` (causa `userRejected` de §4.3) si el `requestId` falta, no existe, ya se resolvio (`status !== 'pending'`) o ha vencido (`expiresAt <= now`): la ventana pinta ese error, declara el hueco y deja «Conectar» deshabilitado |

**Forma final de `wallet_getState` y entrega de la solicitud de conexion (v1.7).** La v1.7 cierra los dos datos que la UI no podia obtener sin vulnerar RNF-14:

1. **`wallet_getState.connectedSites`** es un `ConnectedSiteView[]` **ordenado por `origin` ascendente**, con el origen **normalizado** (minusculas y sin barra final: clave canonica de §2.7 y valor que se envia a `wallet_revokePermissions`) y con `current = true` solo en la sesion **vigente** (existe, no esta desconectada y `expiresAt > now`; `expiresAt === null` = sin caducidad). La lectura es **pura**: NO renueva `lastUsedAt`, NO purga las vencidas —se entregan con `current: false` para que la UI las muestre como vencidas— y **no publica `tabIds` ni ningun secreto**. Es la fuente de la pestaña «Sitios conectados» (M44), que ademas revoca con `wallet_revokePermissions`.
2. **`wallet_getConnectRequest`** entrega a `connect.html` la solicitud de conexion COMPLETA por el **canal interno** (`chrome.runtime.sendMessage`, el mismo que ya usa el popup): M18 sigue publicando `requestId` (y `origin`) en la URL de la ventana como correlador, pero la **lista de cuentas ofrecidas** —y su orden— ya no se deduce del estado, sino que viaja en `accounts` dentro de la solicitud. La ventana pinta esas cuentas y responde `CONNECT_RESPONSE { requestId, success: true, account, accountIndex }` con `accountIndex = accounts.indexOf(account)` sobre ESA lista, de modo que el indice es coherente por construccion; el Service Worker conserva su respaldo (si el indice recibido no apunta a la misma cuenta, recalcula `indexOf(account)`) y sigue siendo el unico que persiste `truekeate_connected_sites` tras validar la cuenta elegida.

**Reglas del contrato interno (ADT-16).** (a) La union `InternalMethod` es **cerrada**: un metodo `wallet_*` que no figure en ella se responde `4200` y **no** se implementa sin editar este contrato. (b) Ningun metodo interno abre ventana ni crea entrada en `truekeate_pending_requests`; la aprobacion, cuando la hay (revelado y reset), es **confirmacion explicita dentro del propio contexto** (secciones 3.8 y 3.9). (c) Todo error usa la tabla de `diccionario_datos.md` seccion 4.3 con `code` numerico obligatorio. (d) `wallet_getLogs` devuelve `dropped` para hacer visible el descarte por cuota (D-M/ADT-14). (e) **RNF-14**: el popup no lee ni escribe el almacen de la extension; toda lectura de estado pasa por `wallet_getState` y toda mutacion por el metodo interno correspondiente. (f) **v1.7**: la **ventana de conexion** tampoco lee el almacen (RNF-14) —recibe la solicitud con `wallet_getConnectRequest`— y los sitios conectados se pintan **solo** con `wallet_getState.connectedSites`.
...
**v1.7 — cierre de los dos huecos de contrato de H3.** **(1)** §5.1.1 amplia `wallet_getState` con **`connectedSites: ConnectedSiteView[]`**: las sesiones de `truekeate_connected_sites` en la forma canonica de `diccionario_datos.md` §2.7 —`origin` normalizado, `account`, `chainId`, `connectedAt`, `lastUsedAt`, `expiresAt`— mas `current` (sesion vigente), **sin secretos** y sin `tabIds`, ordenadas por `origin`; la lectura es pura (no renueva ni purga) y es la fuente de la pestaña «Sitios conectados» (M44). **(2)** Nuevo metodo interno **`wallet_getConnectRequest`** (`{ requestId }` → `ConnectRequestView`): el Service Worker entrega a `connect.html` la solicitud COMPLETA de `truekeate_connect_request` (§2.9) por el **canal interno** (`chrome.runtime.sendMessage`), de modo que la ventana ya no depende de una lectura aparte; la URL de M18 conserva `requestId` solo como correlador. **(3)** La ventana responde con `account` **y** `accountIndex` calculados sobre esa misma lista (`accounts.indexOf(account)`) y el SW conserva su respaldo por `indexOf(account)`, de forma que un indice incoherente no puede conceder una sesion con un indice que no corresponde a la cuenta autorizada. **(4)** §5.1.1 pasa de **15 a 16** metodos internos y documenta los errores de la entrega (`4001`, causa `userRejected` ya registrada en `diccionario_datos.md` §4.3, si la solicitud falta, ya se resolvio o ha vencido). **Sin claves nuevas** en `chrome.storage.local`, **sin causas nuevas** en el catalogo de errores y **sin cambios** en el resto de secciones de este documento.
